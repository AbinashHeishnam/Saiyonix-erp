...
async function setupRepeatableJobs() {
    ...
    for (const school of schools) {
        await jobQueue.add("ASSIGNMENT_REMINDER", { type: "ASSIGNMENT_REMINDER", schoolId: school.id }, {
            jobId: `assignment-reminder:${school.id}`,
            repeat: { every: 60 * 60 * 1000 },
        });
        await jobQueue.add("NOTIFICATION_MONITOR", { type: "NOTIFICATION_MONITOR", schoolId: school.id }, {
            jobId: `notification-monitor:${school.id}`,
            repeat: { every: 5 * 60 * 1000 },
        });
        await jobQueue.add("PUSH_TOKEN_CLEANUP", { type: "PUSH_TOKEN_CLEANUP", schoolId: school.id }, {
    ...
    setInterval(async () => {
        try {
            ...
            console.log(`[worker:heartbeat] waiting=${counts.waiting} active=${counts.active} failed=${counts.failed} completed=${counts.completed}`);
        }
        catch (error) {
