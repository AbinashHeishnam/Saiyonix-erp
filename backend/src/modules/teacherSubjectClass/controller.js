...
import { createTeacherSubjectClass, deleteTeacherSubjectClass, getTeacherSubjectClassById, getTeacherSubjectClasses, updateTeacherSubjectClass, } from "@/modules/teacherSubjectClass/service";
import { teacherSubjectClassIdSchema, } from "@/modules/teacherSubjectClass/validation";
...
export async function list(req, res, next) {
    try {
        ...
        const { items, total } = await getTeacherSubjectClasses(schoolId, parseFilters(req.query), pagination);
        return success(res, items, "Teacher assignments fetched successfully", 200, buildPaginationMeta(total, pagination));
    }
    catch (error) {
        return next(error);
    }
