...
async function ensureExamWithPublishedResults(schoolId, examId) {
...
export async function computeRanking(schoolId, examId) {
    return withConsoleTime(`ranking:compute:${examId}`, async () => {
        const exam = await ensureExamWithPublishedResults(schoolId, examId);
        const reportCards = (await withTiming("ranking:reportCards", () => prisma.reportCard.findMany(enforceQueryLimits({
        ...
        const examSubjects = (await withTiming("ranking:examSubjects", () => prisma.examSubject.findMany(enforceQueryLimits({
            where: {
                examId,
                exam: { schoolId },
        ...
        const enrollments = (await withTiming("ranking:enrollments", () => prisma.studentEnrollment.findMany(enforceQueryLimits({
            where: {
                classId: { in: classIds },
        ...
        const marks = (await withTiming("ranking:marks", () => prisma.mark.findMany(enforceQueryLimits({
            where: {
        ...
        const reportCardMap = new Map(reportCards.map((row) => [
            row.studentId,
            {
...
export async function recomputeRanking(schoolId, examId, actor) {
...
export async function getRankingForActor(schoolId, examId, actor) {
...
export async function listRankingsForAdmin(schoolId, examId, pagination) {
...
export async function getClassRanking(schoolId, examId, classId) {
    const exam = await prisma.exam.findFirst({
        where: { id: examId, schoolId },
        select: { id: true, academicYearId: true },
    });
    ...
    const reportMap = new Map(reportCards.map((row) => [row.studentId, row]));
    const rankMap = new Map(rankSnapshots.map((row) => [row.studentId, row]));
    ...
    return rows.sort((a, b) => {
        if (a.classRank === null && b.classRank === null)
...
export async function primeRankingCacheForExam(schoolId, examId, studentIds) {
    if (studentIds.length === 0)
        return;
    const version = await getCacheVersion("ranking", examId);
    ...
    for (const chunk of chunks) {
        ...
        await Promise.all(snapshots.map((snapshot) => cacheSet(`ranking:v${version}:${examId}:${snapshot.studentId}`, snapshot, 300)));
    }
}
