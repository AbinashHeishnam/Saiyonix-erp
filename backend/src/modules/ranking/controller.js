...
export async function get(req, res, next) {
    try {
        ...
        if (actor.roleType === "ADMIN" || actor.roleType === "ACADEMIC_SUB_ADMIN" || actor.roleType === "SUPER_ADMIN") {
            ...
            const { items, total } = await listRankingsForAdmin(schoolId, examId, safePagination);
            return success(res, items, "Ranking fetched successfully", 200, buildPaginationMetaWithSync(total, safePagination));
        }
