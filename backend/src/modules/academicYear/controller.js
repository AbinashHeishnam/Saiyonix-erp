...
import { createAcademicYear, deleteAcademicYear, getAcademicYearTransitionMeta, getActiveAcademicYear, getPreviousAcademicYear, getAcademicYearById, listAcademicYears, switchAcademicYear, updateAcademicYear, } from "@/modules/academicYear/service";
...
export async function list(req, res, next) {
    try {
        ...
        return success(res, items, "Academic years fetched successfully", 200, buildPaginationMeta(total, pagination));
    }
    catch (error) {
        return next(error);
    }
...
export async function switchYear(req, res, next) {
    try {
        const schoolId = getSchoolId(req);
...
export async function transitionMeta(req, res, next) {
    try {
        const schoolId = getSchoolId(req);
