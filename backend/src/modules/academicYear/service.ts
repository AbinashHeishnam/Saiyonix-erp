...
async function cloneAcademicYearStructure(params: {
  ...
  const periodScope = await tx.$queryRaw<
  ...
  if (periodTable && periodColumn) {
    ...
    const existingPeriods = await tx.$queryRaw<Array<{ count: number }>>(Prisma.sql`
    ...
    const sourcePeriods = await tx.$queryRaw<Array<{ count: number }>>(Prisma.sql`
      SELECT COUNT(*)::int AS count
      FROM ${tableId}
  ...
  type SourceClass = Prisma.ClassGetPayload<{
    include: {
      sections: { select: { sectionName: true; capacity: true } };
      classSubjects: { select: { subjectId: true; periodsPerWeek: true } };
      classSubjectConfigs: { select: { subjectId: true } };
    };
  }>;

  const sourceClasses: SourceClass[] = await tx.class.findMany({
  ...
  const classSubjectConfigRows: Array<{
    classId: string;
    subjectId: string;
  }> = [];
...
async function resetOperationalData(params: {
  ...
  const [teacherAssignments, timetableSlots, studentAttendance, sectionAttendance] =
    await Promise.all([
  ...
  if (
    ...
    throw new ApiError(
      400,
      "Target academic year already has operational data. Disable resetOperationalData to avoid data loss."
    );
  }
...
export async function getPreviousAcademicYear(schoolId: string) {
  const setting = await prisma.systemSetting.findFirst({
    where: { schoolId, settingKey: "ACADEMIC_YEAR_SWITCH" },
    select: { settingValue: true },
  });
  const value = setting?.settingValue as
    | { fromAcademicYearId?: string | null; toAcademicYearId?: string | null }
    | null;
...
export async function getAcademicYearTransitionMeta(schoolId: string) {
  const setting = await prisma.systemSetting.findFirst({
    where: { schoolId, settingKey: "ACADEMIC_YEAR_SWITCH" },
    select: { settingValue: true },
  });
...
export async function canStudentInteractWithPreviousYear(schoolId: string) {
  const meta = await getAcademicYearTransitionMeta(schoolId);
  return Boolean(meta?.canStudentInteract);
}

export async function canClassTeacherInteractWithPreviousYear(schoolId: string) {
  const meta = await getAcademicYearTransitionMeta(schoolId);
