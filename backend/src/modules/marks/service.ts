...
import { bumpCacheVersion, cacheInvalidateByPrefix } from "@/core/cacheService";
import { safeRedisDel } from "@/core/cache/invalidate";
import { logAudit } from "@/utils/audit";
import type { BulkCreateMarksInput, CreateMarkInput } from "@/modules/marks/validation";
...
export async function createMarksBulk(
  schoolId: string,
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (existingMarks.length > 0) {
      const enrollmentByStudent = new Map(
        enrollments.map((enrollment) => [enrollment.studentId, enrollment.sectionId])
      );
    ...
    const latestEnrollmentByStudent = new Map<
      string,
      { classId: string; sectionId: string; createdAt: Date }
    >();
