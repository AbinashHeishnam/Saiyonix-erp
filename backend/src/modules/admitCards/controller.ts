...
import {
  getAdmitCardPdfForActor,
  getAdmitCardForActor,
  publishAdmitCards,
  setAdmitCardPublishStatus,
  unlockAdmitCard,
  listAdmitCardControls,
} from "@/modules/admitCards/service";
import {
  admitCardQuerySchema,
  admitCardByStudentQuerySchema,
  admitCardStudentParamSchema,
  admitCardControlQuerySchema,
  examIdSchema,
  publishAdmitCardSchema,
  toggleAdmitCardSchema,
...
function parseStudentParams(params: AuthRequest["params"]) {
...
function parseExamQuery(query: AuthRequest["query"]) {
...
export async function unlock(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    ...
    const examId = parseExamId(req.params.examId);
    const data = await unlockAdmitCard(
      schoolId,
      examId,
      req.body.studentId,
      actor,
...
export async function generatePdfs(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    const schoolId = getSchoolId(req);
    const examId = parseExamId(req.params.examId);
    ...
    return success(
      ...
      "Admit card PDF generation queued"
    );
  } catch (error) {
    return next(error);
  }
...
export async function getPdf(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    const schoolId = getSchoolId(req);
    const actor = getActor(req);
    const examId = parseExamId(req.params.examId);
...
export async function getByStudent(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    const schoolId = getSchoolId(req);
    const actor = getActor(req);
    const params = parseStudentParams(req.params);
    const query = parseExamQuery(req.query);
    const data = await getAdmitCardForActor(
      schoolId,
      query.examId,
      actor,
      params.studentId
...
export async function togglePublish(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    ...
    const data = await setAdmitCardPublishStatus(
      schoolId,
      parsed.data.examId,
...
export async function listControls(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    const schoolId = getSchoolId(req);
