...
function buildAdmitCardNumber(examId, studentId) {
...
function buildAdmitCardKey(examId, studentId, salt) {
...
async function getExamOrThrow(client, schoolId, examId) {
...
async function getClassIdsForExam(client, schoolId, examId) {
...
async function getLatestEnrollments(client, schoolId, academicYearId, classIds) {
...
async function getAttendancePercentages(params) {
    ...
    for (const chunk of chunks) {
        ...
        const [totalRows, presentRows] = await Promise.all([
            withTiming("admit:attendance:total", () => client.studentAttendance.groupBy({
                by: ["studentId"],
                where: chunkWhere,
                _count: { _all: true },
            })),
            withTiming("admit:attendance:present", () => client.studentAttendance.groupBy({
                by: ["studentId"],
...
async function getFeePaidMap(params) {
    ...
    const chunks = chunkArray(studentIds, 1000);
    for (const chunk of chunks) {
        const batch = await withTiming("admit:payments", () => client.payment.findMany({
            where: {
                feeTermId: feeTerm.id,
                studentId: { in: chunk },
                status: "PAID",
...
function computeLockReason(attendancePercentage, feePaid, isRegistered) {
    ...
    if (feesPending)
        reasons.push("FEES_PENDING");
    if (notRegistered)
        reasons.push("NOT_REGISTERED");
    reasons.sort();
    const lockReason = reasons.length > 0 ? reasons.join(",") : null;
    return { isLocked: reasons.length > 0, lockReason };
}
async function ensureAdmitCardPublished(client, examId) {
...
async function ensureStudentRegisteredForExam(client, examId, studentId) {
...
export async function computeAdmitCardEligibility(schoolId, examId, client = prisma) {
    return withConsoleTime(`admit:eligibility:${examId}`, async () => {
        ...
        const latestEnrollmentByStudent = await getLatestEnrollments(client, schoolId, exam.academicYearId, classIds);
        const studentIds = Array.from(latestEnrollmentByStudent.keys());
        if (studentIds.length === 0) {
            return new Map();
        }
        const cachedEligibility = new Map();
        const missingStudentIds = [];
        for (const studentId of studentIds) {
            const cached = await cacheGet(`admitEligibility:${studentId}:${examId}`);
        ...
        const feePaidByStudent = new Set();
        ...
        const eligibility = new Map(cachedEligibility);
        const control = await client.admitCardControl.findFirst({
            where: { examId },
            select: { isPublished: true },
        });
        ...
        for (const studentId of missingStudentIds) {
            ...
            const { isLocked, lockReason } = computeLockReason(attendancePct, feePaid, isRegistered);
            const canDownloadAdmit = feePaid && isRegistered && isPublished && attendancePct >= ATTENDANCE_THRESHOLD;
...
export async function generateAdmitCardsForExam(schoolId, examId) {
    return prisma.$transaction(async (tx) => {
        ...
        const data = studentIds
            ...
            return {
                ...
                status: (entry.isLocked ? "LOCKED" : "UNLOCKED"),
...
async function ensureStudentInExamClass(client, schoolId, examId, academicYearId, studentId) {
    const classIds = await getClassIdsForExam(client, schoolId, examId);
    if (classIds.length === 0) {
...
export async function unlockAdmitCard(schoolId, examId, studentId, actor, reason) {
    ensureActor(actor);
...
async function resolveStudentForActor(schoolId, actor, studentIdFromQuery) {
...
export async function getAdmitCardForActor(schoolId, examId, actor, studentIdFromQuery) {
...
export async function generateAdmitCardPDF(schoolId, examId, studentId) {
    const exam = await getExamOrThrow(prisma, schoolId, examId);
    const control = await prisma.admitCardControl.findUnique({
        where: { examId },
    });
    ...
    try {
        ...
        const timetable = examSubjects.flatMap((subject) => subject.timetable.map((slot) => ({
...
export async function getAdmitCardPdfForActor(schoolId, examId, actor, studentIdFromQuery) {
    const exam = await getExamOrThrow(prisma, schoolId, examId);
    if (!exam.isPublished) {
        throw new ApiError(400, "Exam is not published");
    }
    ...
    const studentId = await resolveStudentForActor(schoolId, actor, studentIdFromQuery);
    const classId = await ensureStudentInExamClass(prisma, schoolId, examId, exam.academicYearId, studentId);
...
export async function bulkGeneratePDFs(schoolId, examId) {
    const exam = await getExamOrThrow(prisma, schoolId, examId);
    if (!exam.isPublished) {
        throw new ApiError(400, "Exam is not published");
    }
    ...
    for (const chunk of chunks) {
        ...
        await Promise.all(chunk.map((entry) => limit(() => generateAdmitCardPDF(schoolId, examId, entry.studentId))));
    }
    return { count: unlocked.length };
}
export async function publishAdmitCards(schoolId, examId) {
    const exam = await getExamOrThrow(prisma, schoolId, examId);
    ...
    await Promise.all(registrations.map((row) => cacheInvalidateByPrefix(`admitEligibility:${row.studentId}:${examId}`)));
    try {
...
export async function setAdmitCardPublishStatus(schoolId, examId, isPublished) {
    const exam = await getExamOrThrow(prisma, schoolId, examId);
    const control = await prisma.admitCardControl.upsert({
        where: { examId },
        update: {
...
export async function listAdmitCardControls(schoolId, examId) {
