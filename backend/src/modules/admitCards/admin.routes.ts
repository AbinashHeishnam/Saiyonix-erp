...
admitCardAdminRouter.post(
  "/publish",
  authMiddleware,
  allowRoles("SUPER_ADMIN", "ADMIN", "ACADEMIC_SUB_ADMIN"),
  requirePermission("admitCard:publish"),
  validate({ body: publishAdmitCardSchema }),
  publish
);

admitCardAdminRouter.patch(
  "/publish",
  authMiddleware,
  allowRoles("SUPER_ADMIN", "ADMIN", "ACADEMIC_SUB_ADMIN"),
  requirePermission("admitCard:publish"),
  validate({ body: toggleAdmitCardSchema }),
  togglePublish
);

admitCardAdminRouter.get(
  "/controls",
  authMiddleware,
  allowRoles("SUPER_ADMIN", "ADMIN", "ACADEMIC_SUB_ADMIN"),
  requirePermission("admitCard:publish"),
  validate({ query: admitCardControlQuerySchema }),
  listControls
);
