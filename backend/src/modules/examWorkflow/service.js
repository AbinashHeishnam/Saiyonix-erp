...
function calculateAttendancePercentage(total, present) {
    if (total === 0)
        return 0;
...
async function ensureTeacherCanEnterMarks(params) {
    ...
    const classSubjectId = await resolveClassSubjectId(params.schoolId, params.classId, params.subjectId);
...
export async function getTeacherAssignedExams(schoolId, actor) {
    ...
    const subjectAssignmentMap = new Map();
    ...
    for (const examSubject of examSubjects) {
        ...
        const classTeacherSectionsForClass = classTeacherSections.filter((section) => section.classId === examSubject.classSubject.classId);
        ...
        for (const assignment of subjectAssignments) {
            if (assignment.classSubjectId !== examSubject.classSubjectId)
                continue;
    ...
    if (sectionPairs.length > 0) {
        ...
        const submittedMap = new Map(counts.map((row) => [`${row.examSubjectId}:${row.sectionId}`, row._count._all]));
...
export async function getMarksEntryContext(schoolId, query, actor) {
    const teacherId = await resolveTeacherId(schoolId, actor);
    const academicYearId = await getActiveAcademicYearId(schoolId);
    ...
    const classSubjectId = await resolveClassSubjectId(schoolId, query.classId, query.subjectId);
    ...
    const existingMarks = studentIds.length
        ...
        : [];
    const markMap = new Map(existingMarks.map((mark) => [mark.studentId, mark]));
    const marksStatus = studentIds.length > 0 && existingMarks.length === studentIds.length ? "SUBMITTED" : "PENDING";
    return {
...
export async function getMarksEntryMatrix(schoolId, query, actor) {
    const teacherId = await resolveTeacherId(schoolId, actor);
    const academicYearId = await getActiveAcademicYearId(schoolId);
...
export async function submitMarksBulk(schoolId, payload, actor) {
    let updatedSubjects = 0;
    let skippedSubjects = 0;
    for (const subject of payload.subjects) {
        const result = await submitMarks(schoolId, {
...
export async function submitMarks(schoolId, payload, actor) {
    const teacherId = await resolveTeacherId(schoolId, actor);
    const academicYearId = await getActiveAcademicYearId(schoolId);
    ...
    const existingMap = new Map(existingMarks.map((mark) => [
        mark.studentId,
        {
            marksObtained: Number(mark.marksObtained),
            isAbsent: mark.isAbsent,
...
export async function getExamResultStatus(schoolId, examId, actor) {
    ...
    const classIds = Array.from(new Set(examSubjects
    ...
    sections.forEach((section) => {
        if (!sectionByClass.has(section.classId))
            sectionByClass.set(section.classId, []);
        sectionByClass.get(section.classId)?.push(section);
    });
    const enrollmentCountMap = new Map(enrollmentCounts.map((row) => [row.sectionId, row._count._all]));
    const markCountMap = new Map(markCounts.map((row) => [`${row.examSubjectId}:${row.sectionId}`, row._count._all]));
...
export async function publishExamResult(schoolId, examId, actor) {
    const { userId, roleType } = ensureActor(actor);
    if (!isAdminRole(roleType)) {
        throw new ApiError(403, "Forbidden");
    }
...
export async function getExamResultForActor(schoolId, examId, actor) {
    return getReportCardForActor(schoolId, examId, actor);
}
export async function createResultRecheckComplaint(schoolId, payload, actor) {
    const { userId, roleType } = ensureActor(actor);
    let studentId = null;
    let parentId = null;
    if (roleType === "STUDENT") {
...
export async function listComplaints(schoolId, actor, filters) {
    const { roleType } = ensureActor(actor);
    if (!isAdminRole(roleType)) {
        throw new ApiError(403, "Forbidden");
    }
...
export async function getTeacherExamAnalytics(schoolId, params, actor) {
    const { roleType } = ensureActor(actor);
    let teacherId = null;
    if (!isAdminRole(roleType)) {
    ...
    const subjectStats = examSubjects.map((subject) => {
        ...
        for (const studentId of studentIds) {
            ...
            total += value;
            count += 1;
            if (value < Number(subject.passMarks))
                failCount += 1;
        }
        return {
            examSubjectId: subject.id,
    ...
    const students = enrollments.map((enrollment) => {
        ...
        const attendancePct = calculateAttendancePercentage(totalByStudent.get(enrollment.studentId) ?? 0, presentByStudent.get(enrollment.studentId) ?? 0);
        return {
            studentId: enrollment.studentId,
            fullName: enrollment.student.fullName,
            rollNumber: enrollment.rollNumber,
    ...
    const hasSectionRanks = students.some((student) => student.sectionRank !== null);
    if (!hasSectionRanks) {
...
export async function getTeacherMyClassAnalytics(schoolId, params, actor) {
    const { roleType } = ensureActor(actor);
    let teacherId;
    if (isAdminRole(roleType)) {
        if (!params.teacherId) {
    ...
    return getTeacherExamAnalytics(schoolId, {
