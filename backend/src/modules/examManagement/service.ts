import { ExamShift, ExamType, Prisma } from "@prisma/client";
...
import type {
  AddExamScheduleInput,
  AddRoomAllocationInput,
  CreateExamAdminInput,
  DeleteExamScheduleInput,
  SetFinalExamInput,
} from "@/modules/examManagement/validation";
...
async function resolveStudentContext(
  schoolId: string,
  actor: ActorContext
): Promise<{ studentId: string; classId: string; sectionId: string; rollNumber: number | null }> {
...
export async function addExamSchedule(
  schoolId: string,
  payload: AddExamScheduleInput,
  actor: ActorContext
) {
  const { userId, roleType } = ensureActor(actor);
  if (!isAdminRole(roleType)) {
  ...
  const dateKey = (date: Date) => normalizeDate(date).toISOString();
  ...
  const created = await prisma.$transaction(async (tx) => {
    const db = tx as DbClient;
    const results: Array<{ examSubjectId: string; examDate: Date }> = [];
    ...
    for (const item of payload.schedules) {
      ...
      const examSubject =
        (await tx.examSubject.findFirst({
...
export async function deleteExamSchedule(
  schoolId: string,
  payload: DeleteExamScheduleInput,
  actor: ActorContext
) {
  const { userId, roleType } = ensureActor(actor);
  if (!isAdminRole(roleType)) {
...
export async function addRoomAllocations(
  schoolId: string,
  payload: AddRoomAllocationInput,
  actor: ActorContext
) {
  const { userId, roleType } = ensureActor(actor);
  if (!isAdminRole(roleType)) {
...
export async function publishExamAdmin(
  schoolId: string,
  examId: string,
  actor: ActorContext
) {
...
export async function unlockExamAdmin(
  schoolId: string,
  examId: string,
  actor: ActorContext
) {
...
export async function setFinalExamAdmin(
  schoolId: string,
  examId: string,
  payload: SetFinalExamInput,
  actor: ActorContext
) {
  const { userId, roleType } = ensureActor(actor);
  if (!isAdminRole(roleType)) {
...
export async function deleteExamAdmin(
  schoolId: string,
  examId: string,
  actor: ActorContext
) {
  ...
  const [marksCount, reportCardsCount, admitCardsCount, rankSnapshotsCount] =
...
export async function getExamRoutineForStudent(
  schoolId: string,
  actor: ActorContext
) {
  ...
  const examIds = Array.from(
    new Set(schedules.map((item) => item.examSubject.exam.id))
  );
