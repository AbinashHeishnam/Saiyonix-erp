...
import type {
  ...
  DeleteTimetableSlotInput,
  SubstituteTimetableInput,
...
async function getTimetableLocked(client: DbClient, schoolId: string) {
  const setting = await client.systemSetting.findFirst({
    where: { schoolId, settingKey: "TIMETABLE_LOCKED" },
    select: { settingValue: true },
  });
  if (setting?.settingValue === true) return true;
...
async function ensureTimetableNotLocked(client: DbClient, schoolId: string) {
...
async function getEffectiveFromForSection(
  client: DbClient,
  sectionId: string,
  academicYearId: string,
  date: Date
...
async function getEffectiveFromBySection(
  client: DbClient,
  sectionIds: string[],
  academicYearId: string,
  date: Date
) {
  if (sectionIds.length === 0) return new Map<string, Date>();
  const rows = await client.timetableSlot.groupBy({
    by: ["sectionId"],
    where: {
...
async function ensureSubjectBelongsToSchool(
  client: DbClient,
  schoolId: string,
  subjectId: string
) {
...
async function getConfiguredSubjectIds(client: DbClient, classId: string) {
...
async function ensureClassSubjectForSlot(
  client: DbClient,
  params: { schoolId: string; classId: string; subjectId: string }
) {
...
async function validateSlot(
  client: DbClient,
  schoolId: string,
  params: ValidateTimetableSlotInput
) {
  ...
  if (existingSectionSlot) {
    const sameSlot =
      (classSubject?.id ? existingSectionSlot.classSubjectId === classSubject.id : true) &&
...
export async function validateTimetableSlot(
  schoolId: string,
  payload: ValidateTimetableSlotInput
) {
  try {
...
export async function bulkCreateTimetable(
  schoolId: string,
  payload: BulkCreateTimetableInput
) {
...
function groupByDay(slots: Array<any>) {
  ...
  Object.values(grouped).forEach((arr) =>
    arr.sort((a, b) => (a.period?.periodNumber ?? 0) - (b.period?.periodNumber ?? 0))
  );
  return grouped;
}
...
export async function getSectionTimetable(
  ...
  const effectiveFrom = await getEffectiveFromForSection(
    prisma,
    sectionId,
...
export async function getTeacherTodaySchedule(schoolId: string, userId: string) {
  ...
  const filteredForTeacher = Array.from(slotMap.values()).filter(
...
export async function getStudentTimetableForUser(
  schoolId: string,
  userId: string,
  date?: string
) {
...
export async function getParentTimetableForUser(
  schoolId: string,
  userId: string,
  date?: string
) {
  ...
  for (const link of parent.studentLinks) {
    ...
    if (previousYearId && enrollment?.studentId) {
      ...
      if (previousEnrollment) {
        ...
        if (!effectiveEnrollment.sectionId) continue;
        const dateOnly = await resolveTargetDate(prisma, schoolId, date);
        const effectiveFrom = await getEffectiveFromForSection(
          prisma,
          effectiveEnrollment.sectionId,
          effectiveEnrollment.academicYearId,
          dateOnly
        );
        if (!effectiveFrom) {
          results.push({
...
export async function getTimetableLockStatus(schoolId: string) {
  const locked = await getTimetableLocked(prisma, schoolId);
  return { locked };
}

export async function setTimetableLock(schoolId: string, locked: boolean) {
...
export async function getTimetableWorkload(schoolId: string) {
  ...
  const threshold =
    typeof thresholdSetting?.settingValue === "number"
  ...
  const teacherIds = grouped.map((g) => g.teacherId).filter(Boolean) as string[];
  ...
  return grouped.map((item) => ({
    ...
    teacherName: nameMap.get(item.teacherId as string) ?? "Teacher",
...
export async function getTimetableMeta(schoolId: string, sectionId: string) {
  const section = await ensureSectionBelongsToSchool(prisma, schoolId, sectionId);
...
export async function deleteTimetableSlot(
  schoolId: string,
  payload: DeleteTimetableSlotInput
) {
  ...
  const effectiveFrom =
    payload.effectiveFrom
      ...
      : await getEffectiveFromForSection(
          prisma,
          payload.sectionId,
          section.academicYearId,
          dateOnly
...
export async function createSubstitution(
  schoolId: string,
  payload: SubstituteTimetableInput
) {
  await ensureTimetableNotLocked(prisma, schoolId);
