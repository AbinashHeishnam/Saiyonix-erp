...
async function getSchoolTimeZone(client, schoolId) {
...
async function resolveDateRange(client, schoolId, input) {
...
export async function createAcademicCalendarEvent(schoolId, payload, actorUserId) {
...
export async function updateAcademicCalendarEvent(schoolId, eventId, payload, actorUserId) {
    return prisma.$transaction(async (tx) => {
        const db = tx;
...
export async function deleteAcademicCalendarEvent(schoolId, eventId) {
    return prisma.$transaction(async (tx) => {
        const existing = await tx.academicCalendarEvent.findFirst({
            where: { id: eventId, schoolId },
        });
...
export async function createEmergencyHoliday(schoolId, payload, actorUserId) {
    const title = payload.title?.trim() || "Emergency Holiday";
    return createAcademicCalendarEvent(schoolId, {
        academicYearId: payload.academicYearId,
        title,
...
export async function getSessionStartDate(client, academicYearId) {
...
export async function getSessionEndDate(client, academicYearId) {
    const db = client ?? prismaClient;
    const event = await db.academicCalendarEvent.findFirst({
