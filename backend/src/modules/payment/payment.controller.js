...
import { applyGatewayPaymentUpdate, createPaymentOrder, createPaymentLog, createManualPayment, findPaymentByGatewayOrderId, generateAdminReceiptPdf, listPayments, listPaymentLogs, verifyPaymentSignature, } from "@/modules/payment/payment.service";
...
export async function createOrder(req, res, next) {
    try {
        ...
        const resolvedStudentId = await resolveStudentId(schoolId, actor, req.body?.studentId ?? null);
...
export async function getRazorpayKey(req, res, next) {
    try {
        const config = await getRazorpayConfig();
        return success(res, { keyId: config.keyId ?? null }, "Razorpay key fetched");
    }
    catch (error) {
        return next(error);
    }
...
export async function verify(req, res, next) {
    try {
        ...
        return success(res, { verified: true, paymentStatus: result.payment.status }, "Payment verified");
    }
    catch (error) {
        return next(error);
    }
...
export async function list(req, res, next) {
    try {
        ...
        return success(res, items, "Payments fetched successfully", 200, buildPaginationMeta(total, pagination));
    }
    catch (error) {
        return next(error);
    }
...
export async function downloadAdminReceipt(req, res, next) {
    try {
        const schoolId = getSchoolId(req);
...
export async function createManualPaymentRecord(req, res, next) {
    try {
        const schoolId = getSchoolId(req);
        const actor = getActor(req);
