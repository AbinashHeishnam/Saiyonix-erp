...
import { canClassTeacherInteractWithPreviousYear, canStudentInteractWithPreviousYear, getPreviousAcademicYear, } from "@/modules/academicYear/service";
...
async function getStudentIdsForActor(schoolId, actor) {
...
async function hasPreviousYearRelation(schoolId, actor, otherUserId) {
...
async function getAllowedClassTeacherUserIds(schoolId, actor) {
    const studentIds = await getStudentIdsForActor(schoolId, actor);
    ...
    sections.forEach((section) => {
        if (section.classTeacher?.userId)
            ids.add(section.classTeacher.userId);
    });
    subjectTeachers.forEach((link) => {
        if (link.teacher?.userId)
            ids.add(link.teacher.userId);
    });
    timetableTeachers.forEach((slot) => {
        if (slot.teacher?.userId)
            ids.add(slot.teacher.userId);
    });
    if (await canStudentInteractWithPreviousYear(schoolId)) {
        ...
        if (previousYear) {
            ...
            prevClasses.forEach((cls) => {
                if (cls.classTeacher?.userId)
                    ids.add(cls.classTeacher.userId);
            });
        }
    }
    return [...ids];
}
async function getTeacherAllowedUserIds(schoolId, actor) {
...
function buildThreadSubject(userA, userB) {
    const [first, second] = [userA, userB].sort();
    return `chat:${first}:${second}`;
}
async function getOrCreateThread(schoolId, userA, userB) {
...
export async function startChatThread(schoolId, actor, receiverId) {
    if (!actor.userId || !actor.roleType) {
        throw new ApiError(401, "Unauthorized");
    }
    if (actor.roleType === "STUDENT" || actor.roleType === "PARENT") {
...
export async function sendMessage(schoolId, actor, payload) {
    if (!actor.userId || !actor.roleType) {
        throw new ApiError(401, "Unauthorized");
    }
    if (actor.roleType === "STUDENT" || actor.roleType === "PARENT") {
...
export async function getConversation(schoolId, actor, otherUserId, pagination) {
    if (!actor.userId || !actor.roleType) {
        throw new ApiError(401, "Unauthorized");
    }
    if (actor.roleType === "STUDENT" || actor.roleType === "PARENT") {
...
export async function getTeacherContacts(schoolId, actor) {
    if (!actor.userId || actor.roleType !== "TEACHER") {
        throw new ApiError(403, "Forbidden");
    }
    const teacher = await prisma.teacher.findFirst({
    ...
    if (await canClassTeacherInteractWithPreviousYear(schoolId)) {
        ...
        if (previousYear) {
            ...
            previousYearStudentIds.push(...previousEnrollments.map((enrollment) => enrollment.studentId));
        }
    }
...
export async function getTeacherUnreadMessages(schoolId, actor) {
    if (!actor.userId || actor.roleType !== "TEACHER") {
        throw new ApiError(403, "Forbidden");
    }
    const teacher = await prisma.teacher.findFirst({
...
export async function getTeacherUnreadSummary(schoolId, actor) {
    ...
    return [...bySender.values()].sort((a, b) => b.lastSentAt.getTime() - a.lastSentAt.getTime());
}
