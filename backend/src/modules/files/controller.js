...
export async function secureFileAccess(req, res, next) {
    let debugFileUrl = "";
    ...
    let debugStorage = "";
    let debugUserId = "";
    const LOG_FILE_DEBUG = process.env.LOG_FILE_ACCESS === "true" || process.env.NODE_ENV !== "production";
    try {
        const user = req.user;
        const fileUrlRaw = typeof req.query.fileUrl === "string"
            ? req.query.fileUrl
        ...
        const isClassroomFile = normalizedPath.includes("/assignments/") ||
        ...
        async function teacherHasClassSubjectAccess(teacherId, classSubjectId, sectionId) {
        ...
        async function studentHasClassAccess(studentId, classId, sectionId) {
            ...
            if (!enrollment)
                return false;
            if (sectionId && enrollment.sectionId !== sectionId)
                return false;
            return true;
        }
        async function resolveStudentIdForUser(userIdValue) {
        ...
        async function parentHasClassAccess(parentUserId, classId, sectionId) {
        ...
        async function teacherHasStudentAccess(teacherId, studentId) {
            ...
            const sectionIds = Array.from(new Set(enrollments.map((e) => e.sectionId).filter(Boolean)));
        ...
        async function teacherIsClassTeacherForStudent(teacherUserId, studentId) {
        ...
        async function resolveStudentTargetsForUser(userIdValue) {
        ...
        async function resolveParentTargetsForUser(userIdValue) {
            const academicYearId = await getActiveAcademicYearId();
            if (!academicYearId)
                return { classIds: [], sectionIds: [] };
            const parent = await prisma.parent.findFirst({
        ...
        async function ensureNoticeAttachmentAccess() {
            if (isAdmin)
                return true;
            const noticeRows = await prisma.$queryRaw `SELECT "id", "targetType", "targetRole", "targetClassId", "targetSectionId", "publishedAt", "expiresAt"
        FROM "NoticeBoard"
        ...
        async function ensureClassroomFileAccess() {
            ...
            if (assignmentSubmission?.assignment) {
                ...
                if (roleType === "TEACHER") {
                    ...
                    if (!teacher)
                        return false;
                    return teacherHasClassSubjectAccess(teacher.id, assignmentSubmission.assignment.classSubjectId, sectionId);
                }
            }
            ...
            if (note) {
                ...
                if (roleType === "TEACHER") {
                    ...
                    if (!teacher)
                        return false;
                    return (note.teacherId === teacher.id ||
            ...
            const attachmentAssignments = await prisma.$queryRaw `SELECT "id", "teacherId", "classSubjectId", "sectionId"
        FROM "Assignment"
        ...
        LIMIT 1`;
            ...
            if (attachmentAssignment) {
                ...
                if (roleType === "TEACHER") {
                    ...
                    if (!teacher)
                        return false;
                    return (attachmentAssignment.teacherId === teacher.id ||
            ...
            if (chatMessage?.roomId) {
                ...
                if (isAdmin)
                    return true;
                const allowed = await canJoinChatRoomSafe(schoolId, { userId, roleType }, { roomId: room.id, classId: room.classId, sectionId: room.sectionId, subjectId: room.subjectId });
                return allowed;
            }
            return false;
        }
        ...
        const extractInnerFileUrl = (value) => {
            try {
                const parsed = new URL(value);
        ...
        const chatLookupCandidates = Array.from(new Set([normalizedPath, fileUrl, fileUrlRaw, extractInnerFileUrl(fileUrl), extractInnerFileUrl(fileUrlRaw)]
        ...
        if (chatMessage) {
            const chatRoomMemberModel = prisma.chatRoomMember;
            const hasMemberModel = typeof chatRoomMemberModel?.findFirst === "function";
            const isMemberFromModel = hasMemberModel
          ...
          `;
            ...
            const allowed = memberAuthAvailable
                ...
                : isAdmin
                    ? true
                    : await canJoinChatRoomSafe(schoolId, { userId, roleType }, {
                        roomId: chatMessage.roomId,
