...
async function ensureDeleteAllowed(actor, fileUrl) {
    ...
    const ensuredUserId = actorUserId;
    const ensuredRoleType = actorRoleType;
    const ensuredSchoolId = actorSchoolId;
    ...
    const isClassroomFile = normalizedUrl.includes("/assignments/") ||
...
export async function uploadFileForModule(file, payload) {
    if (!file) {
...
export async function deleteStoredFile(fileUrl, actor) {
    if (!fileUrl) {
