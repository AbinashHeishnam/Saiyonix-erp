...
async function getExamOrThrow(schoolId: string, examId: string) {
...
export async function computeResultsForExam(
  schoolId: string,
  examId: string,
  ...
  return withConsoleTime(`results:compute:${examId}`, async () => {
    ...
    const examSubjects = (await withTiming("results:examSubjects", () =>
      client.examSubject.findMany(enforceQueryLimits({
        where: {
          examId,
          exam: { schoolId },
    ...
    )) as unknown as ExamSubjectRow[];

    if (examSubjects.length === 0) {
      return [];
    }

    const examSubjectById = new Map(
      examSubjects.map((subject) => [subject.id, subject])
    );

    const examSubjectsByClass = new Map<string, string[]>();
    ...
    const enrollments = (await withTiming("results:enrollments", () =>
      client.studentEnrollment.findMany(enforceQueryLimits({
        where: {
          classId: { in: classIds },
    ...
    const latestEnrollmentByStudent = new Map<string, { classId: string }>();
    ...
    for (const chunk of studentChunks) {
      const marks = (await withTiming("results:marksChunk", () =>
        client.mark.findMany(enforceQueryLimits({
          where: {
      ...
      const marksByStudent = new Map<string, Map<string, Prisma.Decimal>>();
...
export async function upsertResultsForExam(
  schoolId: string,
  examId: string
) {
  return prisma.$transaction(async (tx) => {
    ...
    for (const chunk of chunks) {
      const operations = chunk.map((result) =>
        tx.reportCard.upsert({
...
export async function recomputeResults(
  schoolId: string,
  examId: string,
  actor: ActorContext
) {
...
export async function getResultsForStudentOrParent(
  schoolId: string,
  examId: string,
  actor: ActorContext
) {
...
export async function getResultsForAdmin(
  schoolId: string,
  examId: string,
  studentId: string
) {
...
async function prewarmExamCaches(schoolId: string, examId: string) {
  try {
