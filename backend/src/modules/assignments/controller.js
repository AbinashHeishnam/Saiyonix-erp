...
import { addAssignmentAttachment as addAssignmentAttachmentService, createAssignment as createAssignmentService, deleteAssignment as deleteAssignmentService, getAssignmentById as getAssignmentByIdService, getSubmissionStatus as getSubmissionStatusService, getSubmissionsForAssignment as getSubmissionsForAssignmentService, gradeSubmission as gradeSubmissionService, listAssignments as listAssignmentsService, submitAssignment as submitAssignmentService, updateAssignment as updateAssignmentService, } from "@/modules/assignments/service";
import { assignmentIdSchema, listAssignmentQuerySchema, submissionIdSchema, } from "@/modules/assignments/validation";
...
export async function list(req, res, next) {
    try {
        ...
        const { items, total } = await listAssignmentsService(schoolId, filters, actor, safePagination);
        return success(res, items, "Assignments fetched successfully", 200, buildPaginationMetaWithSync(total, safePagination));
    }
    catch (error) {
        return next(error);
    }
...
export async function addAttachment(req, res, next) {
    try {
        ...
        const fileName = req.file?.originalname ??
            "attachment";
        const data = await addAssignmentAttachmentService(schoolId, assignmentId, {
            fileName,
...
export async function listSubmissions(req, res, next) {
    try {
        ...
        const id = parseId(req.params.id);
        const { items, total } = await getSubmissionsForAssignmentService(schoolId, id, actor, safePagination);
        return success(res, items, "Submissions fetched successfully", 200, buildPaginationMetaWithSync(total, safePagination));
    }
    catch (error) {
        return next(error);
    }
...
export async function submissionStatus(req, res, next) {
    try {
        const schoolId = getSchoolId(req);
        const actor = getActor(req);
        const assignmentId = parseId(req.params.id);
