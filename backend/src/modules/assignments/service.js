...
async function resolveStudentContextForActor(schoolId, actor, preferredStudentId) {
    const { userId, roleType } = ensureActor(actor);
    if (roleType === "STUDENT") {
...
async function ensureAssignmentExists(schoolId, id) {
...
export async function createAssignment(schoolId, payload, actor) {
...
export async function listAssignments(schoolId, filters, actor, pagination) {
    const { roleType } = ensureActor(actor);
    let where;
    let submissionsStudentId = null;
...
export async function getAssignmentById(schoolId, id, actor) {
    const { roleType } = ensureActor(actor);
    if (roleType === "STUDENT" || roleType === "PARENT") {
        const studentContext = await resolveStudentContextForActor(schoolId, actor);
        const assignment = await prisma.assignment.findFirst({
...
export async function updateAssignment(schoolId, id, payload, actor) {
    const teacherId = await resolveTeacherIdForActor(schoolId, actor);
...
export async function deleteAssignment(schoolId, id, actor) {
    const teacherId = await resolveTeacherIdForActor(schoolId, actor);
    const assignment = await ensureAssignmentExists(schoolId, id);
    if (assignment.teacherId !== teacherId) {
        throw new ApiError(403, "Forbidden");
...
export async function addAssignmentAttachment(schoolId, assignmentId, payload, actor) {
    const teacherId = await resolveTeacherIdForActor(schoolId, actor);
...
export async function submitAssignment(schoolId, assignmentId, payload, actor, preferredStudentId) {
    const studentContext = await resolveStudentContextForActor(schoolId, actor, preferredStudentId);
    const assignment = await prisma.assignment.findFirst({
        where: {
            id: assignmentId,
            classSubject: {
...
export async function getSubmissionsForAssignment(schoolId, assignmentId, actor, pagination) {
    const teacherId = await resolveTeacherIdForActor(schoolId, actor);
    const assignment = await prisma.assignment.findFirst({
        where: {
            id: assignmentId,
...
export async function checkAndSendAssignmentReminders(schoolId) {
    const now = new Date();
    ...
    const classIds = Array.from(new Set(assignments.map((assignment) => assignment.classSubject.classId)));
    ...
    const enrollmentsByClassSection = new Map();
    ...
    for (const assignment of assignments) {
        ...
        const candidateIds = assignment.sectionId === null || assignment.sectionId === undefined
            ? enrollmentsByClass.get(classId) ?? []
            : enrollmentsByClassSection.get(`${classId}:${assignment.sectionId}`) ?? [];
        if (candidateIds.length === 0) {
            continue;
        }
        const submittedSet = submittedByAssignment.get(assignment.id) ?? new Set();
...
export async function gradeSubmission(schoolId, submissionId, payload, actor) {
    const teacherId = await resolveTeacherIdForActor(schoolId, actor);
    ...
    if (payload.marksAwarded !== undefined && payload.marksAwarded !== null) {
        const maxMarks = submission.assignment.maxMarks && "toNumber" in submission.assignment.maxMarks
...
export async function getSubmissionStatus(schoolId, assignmentId, actor) {
    const { roleType } = ensureActor(actor);
    if (roleType !== "STUDENT" && roleType !== "PARENT") {
        throw new ApiError(403, "Forbidden");
    }
