...
function buildReportCardKey(examId: string, studentId: string) {
  return `report-cards/${examId}/${studentId}.pdf`;
}

function getLocalReportCardPath(examId: string, studentId: string) {
  return path.join(process.cwd(), "storage", "report-cards", examId, `${studentId}.pdf`);
}

function getLocalReportCardUrl(examId: string, studentId: string) {
  ...
  return new Promise((resolve, reject) => {
    ...
    (async () => {
      ...
      if (params.classRank !== null || params.sectionRank !== null || params.schoolRank !== null) {
        ...
        const ranks = [];
        if (params.classRank !== null) ranks.push(`Class: ${params.classRank}`);
        if (params.sectionRank !== null) ranks.push(`Section: ${params.sectionRank}`);
        if (params.schoolRank !== null) ranks.push(`School: ${params.schoolRank}`);
...
export async function getReportCard(
  ...
  const cached = await cacheGet<ReturnType<typeof buildReportCardResponse>>(cacheKey);
  if (cached) {
    return cached;
  }
  ...
  const marks = (await prisma.mark.findMany(enforceQueryLimits({
    ...
    examSubject: {
      ...
      maxMarks: Prisma.Decimal;
      passMarks: Prisma.Decimal;
      classSubject: { subject: { id: string | null; name: string | null } | null };
    };
  }>;
  ...
  const subjects: SubjectResult[] = marks.map((mark) => {
  ...
  const totalMarksValue = subjects.reduce(
    (sum, subject) => sum.plus(subject.marksObtained),
    new Prisma.Decimal(0)
  );
  const totalMaxMarks = subjects.reduce(
    (sum, subject) => sum.plus(subject.maxMarks),
    new Prisma.Decimal(0)
  );
  ...
  const resultStatus =
    subjects.length === 0 || subjects.some((subject) => subject.status === "FAIL")
      ? "FAIL"
      : "PASS";
...
function buildReportCardResponse(params: {
  ...
  totalMarksValue: Prisma.Decimal;
...
export async function getReportCardForActor(
  schoolId: string,
  examId: string,
  actor: ActorContext
) {
...
export async function getReportCardForAdmin(
  schoolId: string,
  examId: string,
  studentId: string,
  actor: ActorContext
...
export async function getReportCardPdfStatus(
  schoolId: string,
  examId: string,
  studentId: string
) {
