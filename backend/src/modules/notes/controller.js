...
import { createNote as createNoteService, deleteNote as deleteNoteService, getNoteById as getNoteByIdService, listNotes as listNotesService, updateNote as updateNoteService, } from "@/modules/notes/service";
...
export async function list(req, res, next) {
    try {
        ...
        const { items, total } = await listNotesService(schoolId, filters, actor, safePagination);
        return success(res, items, "Notes fetched successfully", 200, buildPaginationMetaWithSync(total, safePagination));
    }
    catch (error) {
        return next(error);
    }
