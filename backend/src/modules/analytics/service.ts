...
async function teacherHasStudentAccess(schoolId: string, teacherId: string, studentId: string) {
    ...
    if (enrollment.sectionId) {
        ...
        if (timetableLink) return true;
    }

    return false;
}

export async function getAnalyticsForActor(
    schoolId: string,
    studentIdOption: string,
    ...
    if (studentId === "me" && actor.roleType === "STUDENT") {
        ...
        if (meStudent) studentId = meStudent.id;
        else throw new ApiError(404, "Student mapping not found");
    }
    ...
    if (actor.roleType === "STUDENT") {
        ...
        if (!student) throw new ApiError(403, "Forbidden");
    } else if (actor.roleType === "PARENT") {
        const parent = await prisma.parent.findFirst({
            where: { schoolId, userId: actor.userId },
            select: { id: true },
        });
        if (!parent) throw new ApiError(403, "Parent account not linked");
    ...
    if (!targetExamId) throw new ApiError(404, "No published exams found for analytics");
...
export async function getSchoolAnalytics(schoolId: string) {
    ...
    const admissionTrendMap: Record<string, number> = {};
    ...
    admissions.forEach(a => {
        ...
        if (admissionTrendMap[key] !== undefined) admissionTrendMap[key]++;
    });
    ...
    attendances.forEach(a => {
        ...
        if (!attendanceByDate[d]) attendanceByDate[d] = { present: 0, total: 0 };
        attendanceByDate[d].total++;
        if (a.status === "PRESENT" || a.status === "LATE") attendanceByDate[d].present++;
    });
