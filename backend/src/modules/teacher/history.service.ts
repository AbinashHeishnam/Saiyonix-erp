...
export async function getTeacherHistoryByUserId(schoolId: string, userId: string) {
...
export async function getTeacherHistory(schoolId: string, teacherId: string) {
  const teacher = await prisma.teacher.findFirst({
  ...
  const timelineMap = new Map<
    string,
    {
      academicYear: { id: string; label: string; startDate: Date; endDate: Date };
      subjects: Array<{ classId: string; className: string; sectionName: string | null; subjectName: string; subjectCode: string }>;
      classTeacherAssignments: Array<{ classId: string; className: string; sectionName?: string | null }>;
    }
  >();
  ...
  const timeline = Array.from(timelineMap.values()).sort(
    (a, b) => b.academicYear.startDate.getTime() - a.academicYear.startDate.getTime()
  );
