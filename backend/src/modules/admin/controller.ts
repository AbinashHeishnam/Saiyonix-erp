...
export async function getAdminTeacherDetails(
  req: AuthRequest,
  res: Response,
  next: NextFunction
) {
  try {
    ...
    type AdminTeacherDetails = Prisma.TeacherGetPayload<{
      include: {
        user: {
          select: {
            id: true;
    ...
    const teacher: AdminTeacherDetails | null = await prisma.teacher.findFirst({
    ...
    const assignedSections = timetableSlots
      ...
      .filter(Boolean)
      .reduce((acc, section) => {
        if (!section || acc.has(section.id)) return acc;
        acc.set(section.id, section);
        return acc;
      }, new Map<string, (typeof timetableSlots)[number]["section"]>())
      .values();
    ...
    const subjects = timetableSlots.reduce((acc, slot) => {
      ...
      return acc;
    }, new Map<string, { id: string; className: string | null; sectionName: string | null; subjectName: string | null }>());
...
export async function getAdminStudentDetails(
  req: AuthRequest,
  res: Response,
  next: NextFunction
) {
  try {
    ...
    type AdminStudentDetails = Prisma.StudentGetPayload<{
      include: {
        profile: true;
    ...
    const student: AdminStudentDetails | null = await prisma.student.findFirst({
...
export async function assignSectionRollNumbers(
  req: AuthRequest,
  res: Response,
  next: NextFunction
) {
...
export async function assignClassRollNumbers(
  req: AuthRequest,
  res: Response,
  next: NextFunction
) {
