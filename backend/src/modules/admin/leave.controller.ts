...
import {
  adminUpdateStudentLeaveStatus as adminUpdateStudentLeaveStatusService,
...
function parseStudentLeaveId(id: unknown): string {
  if (typeof id !== "string") {
...
function parseTeacherLeaveId(id: unknown): string {
  if (typeof id !== "string") {
    throw new ApiError(400, "Invalid id");
  }
...
export async function listStudentLeavesAdmin(
  req: AuthRequest,
  res: Response,
  next: NextFunction
) {
  try {
    ...
    return success(
      res,
      items,
      "Student leaves fetched successfully",
      200,
...
export async function listTeacherLeavesAdmin(
  req: AuthRequest,
  res: Response,
  next: NextFunction
) {
  try {
    ...
    return success(
      res,
      items,
      "Teacher leaves fetched successfully",
      200,
      buildPaginationMetaWithSync(total, pagination)
    );
  } catch (error) {
...
export async function updateStudentLeaveAdmin(
  req: AuthRequest,
  res: Response,
  next: NextFunction
) {
  try {
    ...
    const data = await adminUpdateStudentLeaveStatusService(
      schoolId,
      id,
      actor,
      req.body.status,
...
export async function updateTeacherLeaveAdmin(
  req: AuthRequest,
  res: Response,
  next: NextFunction
) {
