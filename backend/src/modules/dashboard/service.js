...
import { getStudentMonthlySummary, getStudentMonthlySummaries, } from "@/modules/attendance/summaries/service";
...
function getLocalDayName(date, timeZone) {
...
async function getStudentByUserId(schoolId, userId) {
...
async function getTeacherByUserId(schoolId, userId) {
...
async function getClassTeacherSections(schoolId, teacherId, academicYearId) {
...
async function getParentByUserId(schoolId, userId) {
...
async function getEnrollmentForYear(schoolId, studentId, academicYearId) {
...
async function getCircularsForTarget(schoolId, filters) {
...
async function getUpcomingExamsForEnrollment(params) {
    ...
    return items.map((item) => {
        ...
        const room = (allocationMap.get(examId) ?? []).find((alloc) => rollNo >= alloc.rollFrom && rollNo <= alloc.rollTo);
        return {
            examId,
...
export async function getStudentDashboard(params) {
    ...
    const dashboardAcademicYear = effectiveEnrollment?.academicYear ?? activeAcademicYear;
    const dashboardAcademicYearId = dashboardAcademicYear.id;
    const enrollment = effectiveEnrollment ??
        (await getEnrollmentForYear(params.schoolId, student.id, activeAcademicYear.id));
...
export async function getTeacherDashboard(params) {
    ...
    const timetable = Array.isArray(timetableRaw)
        ? timetableRaw
        : Array.isArray(timetableRaw?.data)
            ? timetableRaw.data
            : Array.isArray(timetableRaw?.items)
                ? timetableRaw.items
                : [];
...
export async function getParentDashboard(params) {
    ...
    const promotionRows = validStudentIds.length
        ...
        : [];
    const lastPromotionByStudent = new Map();
    ...
    const attendanceSummaries = await Promise.all(validStudentIds.map(async (studentId) => {
    ...
    const todayRecords = await Promise.all(validStudentIds.map(async (studentId) => {
        const enrollment = latestEnrollmentByStudent.get(studentId);
        const academicYearId = enrollment?.academicYearId;
        if (!academicYearId)
            return [studentId, null];
        ...
        return [studentId, record?.status ?? null];
    }));
    ...
    const upcomingExamLists = await Promise.all(enrollments.map((enrollment) => getUpcomingExamsForEnrollment({
        schoolId: params.schoolId,
        classId: enrollment.classId,
        sectionId: enrollment.sectionId,
        rollNumber: enrollment.rollNumber ?? null,
    ...
    const circularsByEnrollment = await Promise.all(enrollments.map((enrollment) => getCircularsForTarget(params.schoolId, {
        classId: enrollment.classId,
        sectionId: enrollment.sectionId,
        roleType: "PARENT",
    })));
    const circularMap = new Map(circularsByEnrollment.flatMap((result) => result.items.map((item) => [item.id, item])));
    ...
    const fallbackCirculars = enrollments.length === 0
