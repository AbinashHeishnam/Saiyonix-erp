...
import { retryOnceOnUniqueOrSerialization } from "@/core/utils/retry";
...
async function resolveAcademicYearId(client, schoolId, academicYearId, academicYearLabel) {
    if (academicYearId) {
...
async function resolveScholarshipPercent(client, params) {
...
async function resolveDiscountCandidates(client, params) {
    const enrollment = await client.studentEnrollment.findFirst({
        where: { studentId: params.studentId, academicYearId: params.academicYearId },
        select: { sectionId: true },
    });
...
async function resolveFeeDueDate(client, academicYearId, classId) {
...
async function ensureStudentBelongsToSchool(client, schoolId, studentId) {
    const student = await client.student.findFirst({
...
async function resolveStudentEnrollment(client, schoolId, studentId, academicYearId) {
    const enrollment = await client.studentEnrollment.findFirst({
        where: {
            studentId,
            academicYearId,
...
async function resolveFeeStructure(client, schoolId, academicYearId, classId, category) {
...
async function computeFeeBreakdown(client, params) {
...
async function ensureFeeRecordTx(tx, studentId, academicYearId, classId, totalAmount) {
    const db = tx;
...
export async function createFeeStructure(input) {
    const academicYearId = await resolveAcademicYearId(prisma, input.schoolId, input.academicYearId ?? null, input.academicYear ?? null);
...
export async function createScholarship(input) {
    ...
    let admissionNumber = normalizedAdmissionNumber;
...
export async function updateScholarship(input) {
    ...
    const academicYearId = await resolveAcademicYearId(prisma, input.schoolId, input.academicYearId ?? existing.academicYearId, input.academicYear ?? null);
    const normalizedAdmissionNumber = input.admissionNumber?.trim() || null;
    const normalizedTitle = input.title?.trim() || "Scholarship";
    const hasAdmission = Boolean(normalizedAdmissionNumber);
    const normalizedClassId = hasAdmission ? null : input.classId ?? null;
...
export async function listLateFeeRecords(input) {
    const academicYearId = await resolveAcademicYearId(prisma, input.schoolId, input.academicYearId ?? null, null);
    const feeTerm = await prisma.feeTerm.findFirst({
        where: { academicYearId },
        orderBy: { termNo: "desc" },
        select: { id: true },
...
export async function assignFeeToStudents(input) {
    ...
    const amountByClass = new Map(structures.map((item) => [item.classId, new Prisma.Decimal(item.amount)]));
    let createdCount = 0;
...
export async function getStudentFeeStatus(input, db) {
    if (!db) {
        throw new Error("DB client not provided");
    }
    await ensureStudentBelongsToSchool(db, input.schoolId, input.studentId);
    const academicYearId = await resolveAcademicYearId(db, input.schoolId, input.academicYearId ?? null, input.academicYear ?? null);
...
export async function payFee(input) {
    ...
    const classId = input.classId ??
        (await resolveStudentEnrollment(prisma, input.schoolId, input.studentId, academicYearId))
            .classId;
    const structure = await resolveFeeStructure(prisma, input.schoolId, academicYearId, classId, DEFAULT_FEE_CATEGORY);
    ...
    let paymentPayload = input.payment?.orderId && input.payment?.paymentId && input.payment?.signature
        ? input.payment
        : null;
    ...
    const result = await retryOnceOnUniqueOrSerialization(async () => prisma.$transaction(async (tx) => {
        const db = tx;
        const txClient = tx;
        const feeRecord = await ensureFeeRecordTx(txClient, input.studentId, academicYearId, classId, new Prisma.Decimal(structure.amount));
...
export async function getFeeOverviewSnapshot(schoolId, academicYearId) {
    ...
    const [totalStudents, feeTotals, paidStudentsGroup, paymentTotals, termTotals, feeTerms, classTotals, classes, paymentMethodTotals, monthlyTrendRows, topDefaultersRows,] = await Promise.all([
    ...
    const termTotalsMap = new Map(termTotals.map((row) => [row.feeTermId, toNumber(row._sum.amount)]));
    ...
    const classWise = classTotals.map((row) => {
        ...
        return {
            className: classMap.get(row.classId) ?? "Class",
