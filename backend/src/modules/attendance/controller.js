...
import { markAttendance, updateAttendance, getAttendanceBlockInfo, getClassTeacherAttendanceContext, } from "@/modules/attendance/service";
...
export async function attendanceContext(req, res, next) {
    try {
        const schoolId = getSchoolId(req);
        const actor = getActor(req);
        ...
        if (todayLocal < windowStart) {
            ...
            const nextStartGuess = new Date(Date.UTC(nextYear, nextMonth - 1, nextDayNum, startH, startM));
...
export async function listAudit(req, res, next) {
    try {
        const schoolId = getSchoolId(req);
        const attendanceId = typeof req.query.attendanceId === "string" ? req.query.attendanceId : undefined;
        const studentId = typeof req.query.studentId === "string" ? req.query.studentId : undefined;
        if ("page" in req.query || "limit" in req.query) {
            console.log("[Phase1] Pagination applied");
        }
        const pagination = parsePagination(req.query);
        const { items, total } = await listAttendanceAuditLogs(schoolId, { attendanceId, studentId }, pagination);
        return success(res, items, "Attendance audit logs fetched successfully", 200, buildPaginationMeta(total, pagination));
    }
    catch (error) {
        return next(error);
    }
}
export async function studentMonthlySummary(req, res, next) {
    try {
        const schoolId = getSchoolId(req);
        const studentId = typeof req.query.studentId === "string" ? req.query.studentId : undefined;
...
export async function schoolSummary(req, res, next) {
    try {
        const schoolId = getSchoolId(req);
        const academicYearId = typeof req.query.academicYearId === "string"
            ? req.query.academicYearId
...
export async function teacherContext(req, res, next) {
    try {
        const schoolId = getSchoolId(req);
