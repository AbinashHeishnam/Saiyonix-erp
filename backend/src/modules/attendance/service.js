...
import { calculateAttendancePercentage, checkAttendanceThresholds, DEFAULT_ATTENDANCE_THRESHOLDS, } from "@/core/risk/attendanceRisk";
...
function isSameLocalDate(a, b, timeZone) {
...
function combineDateAndTime(date, time) {
    return new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate(), time.getUTCHours(), time.getUTCMinutes(), time.getUTCSeconds(), time.getUTCMilliseconds()));
}
...
async function ensureTimetableSlotBelongsToSection(client, schoolId, timetableSlotId, sectionId, academicYearId) {
...
async function ensureTeacherIsClassTeacher(client, schoolId, sectionId, userId, attendanceDate, timeZone) {
...
async function ensureStudentsInSection(client, params) {
...
async function findExistingStudentAttendance(client, params) {
...
async function findSectionAttendance(client, params) {
...
async function getSchoolStartTime(client, schoolId) {
...
async function getAttendanceSettings(client, schoolId) {
    ...
    const windowMinutes = typeof rawWindow === "number"
        ? rawWindow
    ...
    return {
        ...
        warningLevels: warningLevels.length > 0 ? warningLevels : [...DEFAULT_ATTENDANCE_THRESHOLDS],
    };
}
async function getAttendanceCounts(client, schoolId, studentId, academicYearId) {
...
function computePercentage(counts) {
...
async function enforceAttendanceWindow(params) {
    ...
    if (now < windowStart || now > windowEnd) {
        throw new ApiError(403, `Attendance allowed only during school hours (${startTime} - ${endTime})`);
    }
}
export async function markAttendance(schoolId, payload, actor) {
    ...
    const created = await prisma.$transaction(async (tx) => {
        ...
        attendanceDate = toLocalDateOnly(payload.attendanceDate, timeZone, () => new ApiError(400, "Invalid attendanceDate"));
        const academicYearId = payload.academicYearId ?? (await getActiveAcademicYearId(db, schoolId));
        ...
        const teacherId = await ensureTeacherIsClassTeacher(db, schoolId, sectionId, userId, attendanceDate, timeZone);
        ...
        const recordsToCreate = payload.records.filter((record) => !existingSet.has(record.studentId));
        ...
        for (const chunk of recordChunks) {
            const createdChunk = await Promise.all(chunk.map((record) => tx.studentAttendance.create({
                data: {
    ...
    for (const entry of created) {
        ...
        const countsAfter = await getAttendanceCounts(prisma, schoolId, entry.studentId, entry.academicYearId);
        const before = {
    ...
    for (const chunk of auditChunks) {
        await Promise.all(chunk.map((entry) => logAudit({
            userId,
            action: "MARK",
...
export async function updateAttendance(schoolId, attendanceId, payload, actor) {
    const { userId, roleType } = ensureActor(actor);
    if (roleType !== "TEACHER") {
    ...
    if (now < schoolWindow.windowStart || now > schoolWindow.windowEnd) {
        throw new ApiError(403, `Attendance edits allowed only during school hours (${schoolWindow.startTime} - ${schoolWindow.endTime})`);
    }
    await ensureTeacherIsClassTeacher(prisma, schoolId, record.sectionId, userId, record.attendanceDate, timeZone);
    const { warningLevels } = await getAttendanceSettings(prisma, schoolId);
    ...
    if (payload.status && payload.status !== record.status) {
        const countsAfter = await getAttendanceCounts(prisma, schoolId, record.studentId, record.academicYearId);
        const before = {
            total: countsAfter.total,
            present: Math.max(0, countsAfter.present - (isPresentStatus(payload.status) ? 1 : 0) +
...
export async function getClassTeacherAttendanceContext(schoolId, actor) {
    const { userId, roleType } = ensureActor(actor);
    if (roleType !== "TEACHER") {
    ...
    if (sections.length === 0) {
        ...
        const mapped = substitutions
            ...
            if (!item.section)
                return false;
            if (item.isClassTeacherSubstitution)
                return true;
