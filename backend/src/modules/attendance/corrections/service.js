...
async function ensureTeacherIsClassTeacher(schoolId, sectionId, userId, attendanceDate) {
...
export async function requestAttendanceCorrection(schoolId, payload, actor) {
    ...
    await ensureTeacherIsClassTeacher(schoolId, attendance.sectionId, userId, attendance.attendanceDate);
...
async function updateCorrectionStatus(schoolId, correctionId, actor, status, payload) {
    const { userId, roleType } = ensureActor(actor);
    ...
    if (status === "APPROVED") {
        const countsAfter = await getAttendanceCounts(prisma, schoolId, correction.attendance.studentId, correction.attendance.academicYearId);
        const before = {
            total: countsAfter.total,
            present: Math.max(0, countsAfter.present - (isPresentStatus(correction.newStatus) ? 1 : 0) +
...
export async function approveAttendanceCorrection(schoolId, correctionId, actor, payload) {
    return updateCorrectionStatus(schoolId, correctionId, actor, "APPROVED", payload);
}
export async function rejectAttendanceCorrection(schoolId, correctionId, actor, payload) {
