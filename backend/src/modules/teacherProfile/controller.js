...
import { createTeacherProfile, getTeacherProfileByTeacherId, listTeacherProfiles, updateTeacherProfile, } from "@/modules/teacherProfile/service";
...
export async function getByTeacherId(req, res, next) {
    try {
        const schoolId = getSchoolId(req);
...
export async function list(req, res, next) {
    try {
        ...
        return success(res, items, "Teacher profiles fetched successfully", 200, buildPaginationMeta(total, pagination));
    }
    catch (error) {
        return next(error);
    }
