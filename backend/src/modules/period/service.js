...
export async function createPeriod(schoolId, payload) {
    try {
...
export async function listPeriods(schoolId, pagination, _academicYearId) {
...
export async function getPeriodById(schoolId, id) {
...
export async function updatePeriod(schoolId, id, payload) {
...
export async function deletePeriod(schoolId, id) {
    await getPeriodById(schoolId, id);
    try {
...
export async function autoGeneratePeriods(schoolId, payload) {
    ...
    const data = Array.from({ length: payload.periods }).map((_, index) => {
        ...
        const periodStart = startMinutes +
            baseDuration * index +
    ...
    try {
        const created = await prisma.$transaction(data.map((item) => prisma.period.create({
            data: item,
        })));
