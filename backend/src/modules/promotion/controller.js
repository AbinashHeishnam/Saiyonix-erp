...
import { applyFinalPromotion, generatePromotionList, listPromotionTransitions, getPromotionCriteria, getParentPromotionStatus, getRollNumberAssignmentStatus, getStudentPromotionStatus, previewPromotionEligibility, listPromotionRecords, overridePromotion, reviewPromotionOverride, publishPromotion, assignRollNumbers, upsertPromotionCriteria, } from "@/modules/promotion/service";
import { applyFinalPromotionSchema, assignRollNumbersSchema, generatePromotionSchema, listPromotionSchema, overridePromotionSchema, reviewPromotionOverrideSchema, promotionCriteriaSchema, previewPromotionSchema, promotionTransitionSchema, publishPromotionSchema, } from "@/modules/promotion/validation";
...
export async function upsertCriteria(req, res, next) {
    try {
...
export async function reviewOverride(req, res, next) {
    try {
        const schoolId = getSchoolId(req);
...
export async function applyFinal(req, res, next) {
    try {
        const schoolId = getSchoolId(req);
...
export async function rollNumberStatus(req, res, next) {
    try {
        const schoolId = getSchoolId(req);
        const academicYearId = String(req.query.academicYearId ?? "");
        if (!academicYearId) {
...
export async function assignRollNumbersHandler(req, res, next) {
    try {
        const schoolId = getSchoolId(req);
...
export async function studentStatus(req, res, next) {
    try {
        const schoolId = getSchoolId(req);
...
export async function parentStatus(req, res, next) {
    try {
        const schoolId = getSchoolId(req);
