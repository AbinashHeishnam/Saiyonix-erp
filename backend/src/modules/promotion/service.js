...
async function resolveStudentIdForParent(schoolId, userId) {
...
export async function upsertPromotionCriteria(schoolId, payload, actorUserId) {
    const promotionCriteria = prisma.promotionCriteria;
...
export async function getPromotionCriteria(schoolId, academicYearId) {
    const promotionCriteria = prisma
        .promotionCriteria;
    if (!promotionCriteria) {
        throw new ApiError(500, "Promotion criteria model unavailable. Run prisma generate.");
    }
...
async function buildPromotionRecordsForExam(params) {
    const classIds = Array.from(new Set(params.exam.examSubjects.map((subject) => subject.classSubject.classId)));
    ...
    const totalSubjectsByClass = new Map();
    params.exam.examSubjects.forEach((subject) => {
        examSubjectMeta.set(subject.id, {
            classId: subject.classSubject.classId,
            passMarks: subject.passMarks,
        });
        totalSubjectsByClass.set(subject.classSubject.classId, (totalSubjectsByClass.get(subject.classSubject.classId) ?? 0) + 1);
    });
    ...
    return enrollments.map((enrollment) => {
        ...
        for (const mark of studentMarks) {
            ...
            if (!meta || meta.classId !== enrollment.classId)
                continue;
            if (mark.isAbsent)
                continue;
        ...
        const isEligibleByAttendance = attendancePercent >= params.criteria.minAttendancePercent;
...
export async function generatePromotionList(schoolId, payload) {
...
export async function previewPromotionEligibility(schoolId, payload) {
    const criteria = await prisma.promotionCriteria.findUnique({
    ...
    for (const exam of finalExams) {
        ...
        for (const record of records) {
            ...
            const maxAllowedFailures = Math.max(0, record.totalSubjects - criteria.minSubjectPassCount);
            results.push({
...
export async function listPromotionRecords(schoolId, actor, params) {
...
export async function getStudentPromotionStatus(schoolId, actor) {
    const { userId, roleType } = ensureActor(actor);
    if (roleType !== "STUDENT") {
        throw new ApiError(403, "Forbidden");
    }
    ...
    const currentEnrollment = activeAcademicYearId
        ...
        : null;
    const latestEnrollment = currentEnrollment ??
        (await prisma.studentEnrollment.findFirst({
    ...
    const validPromotion = Boolean(promotionRecord?.academicYear?.isLocked || promotionRecord?.academicYear?.isActive);
            ...
            : promotionRecord?.isFinalClass
                ...
                : promotionRecord?.status === "PROMOTED"
                    ...
                    : isAttendanceOk && isSubjectOk
                        ? "ELIGIBLE"
                        : "FAILED";
    const hasAdvancedEnrollment = Boolean(currentEnrollment?.academicYearId) &&
        Boolean(promotionRecord?.academicYearId) &&
        currentEnrollment?.academicYearId !== promotionRecord?.academicYearId;
    let fallbackNextClass = null;
    let fallbackNextSection = null;
    if (!hasAdvancedEnrollment &&
        ...
        const nextYear = activeYear && activeYear.id !== promotionRecord.academicYearId
            ? activeYear
    ...
    return {
        ...
        failedSubjects: validPromotion && failedSubjectsComputed != null
...
export async function getParentPromotionStatus(schoolId, actor) {
    const { userId, roleType } = ensureActor(actor);
    if (roleType !== "PARENT") {
        throw new ApiError(403, "Forbidden");
    }
...
export async function overridePromotion(schoolId, actor, params) {
    const { roleType } = ensureActor(actor);
...
export async function reviewPromotionOverride(schoolId, actor, params) {
...
export async function publishPromotion(schoolId, actor, payload) {
    const { roleType } = ensureActor(actor);
    if (roleType !== "ADMIN" && roleType !== "ACADEMIC_SUB_ADMIN" && roleType !== "SUPER_ADMIN") {
        throw new ApiError(403, "Forbidden");
    }
    ...
    const classToExamId = new Map();
    const duplicateExamClasses = new Set();
    for (const exam of finalExams) {
    ...
    for (const record of computedRecords) {
        ...
        if (!examId) {
            throw new ApiError(400, `Final exam mapping missing for class ${record.classId}`);
        }
    }
...
export async function applyFinalPromotion(schoolId, actor, payload) {
    const { roleType, userId } = ensureActor(actor);
    if (roleType !== "ADMIN" && roleType !== "ACADEMIC_SUB_ADMIN" && roleType !== "SUPER_ADMIN") {
        throw new ApiError(403, "Forbidden");
    }
    ...
    const sectionIdSetByClass = new Map();
    for (const section of targetSections) {
        ...
        const set = sectionIdSetByClass.get(section.classId) ?? new Set();
        set.add(section.id);
    ...
    const existingByStudent = new Map(existingEnrollments.map((entry) => [
        entry.studentId,
    ...
    const reportCardByKey = new Map(reportCards.map((card) => [
    ...
    const promotedStudents = new Set();
    let finalCount = 0;
    ...
    for (const record of promotionRecords) {
        ...
        if (existing) {
            if (existing.classId !== targetClassId) {
                throw new ApiError(400, `Existing enrollment mismatch for student ${record.studentId}`);
            }
            const allowedSections = sectionIdSetByClass.get(targetClassId);
            if (!allowedSections || !allowedSections.has(existing.sectionId)) {
                throw new ApiError(400, `Existing enrollment section invalid for student ${record.studentId}`);
            }
            promotionUpdates.push({
                id: record.id,
                status: finalStatus,
    ...
    await prisma.$transaction(async (tx) => {
        ...
        const filteredEnrollments = enrollmentsToCreate.filter((enrollment) => !existingIds.has(enrollment.studentId));
        ...
        if (remainingPreview > 0) {
            throw new ApiError(500, "Promotion finalization did not normalize all promotion statuses");
        }
    ...
    if (shouldActivate) {
        await switchAcademicYear(schoolId, {
    ...
    for (const update of promotionUpdates) {
        if (update.status !== "PROMOTED" || update.isFinalClass)
            continue;
...
export async function listPromotionTransitions(schoolId, actor, params) {
    const { roleType } = ensureActor(actor);
    if (roleType !== "ADMIN" && roleType !== "ACADEMIC_SUB_ADMIN" && roleType !== "SUPER_ADMIN") {
        throw new ApiError(403, "Forbidden");
    }
    ...
    const enrollmentByStudent = new Map(enrollments.map((e) => [
        e.studentId,
        { class: e.class ?? null, section: e.section ?? null },
    ]));
    const targetClassIds = new Set();
    for (const record of records) {
        if (record.promotedClass?.id)
            targetClassIds.add(record.promotedClass.id);
    }
    for (const enrollment of enrollments) {
        if (enrollment.class?.id)
            targetClassIds.add(enrollment.class.id);
    }
    const targetSections = targetClassIds.size === 0
        ? []
    ...
    return records.map((record) => {
        ...
        const targetSectionsForClass = nextClass?.id
            ...
            : [];
        const hasNameMatch = Boolean(fromSectionName &&
            targetSectionsForClass.some((s) => s.sectionName.toLowerCase() === fromSectionName.toLowerCase()));
        let sectionMapping = "NONE";
        let sectionMappingReason = "NO_TARGET_SECTION";
        if (!fromSectionName) {
            sectionMapping = "NONE";
...
export async function getRollNumberAssignmentStatus(schoolId, actor, academicYearId) {
    const { roleType } = ensureActor(actor);
    if (roleType !== "ADMIN" && roleType !== "ACADEMIC_SUB_ADMIN" && roleType !== "SUPER_ADMIN") {
        throw new ApiError(403, "Forbidden");
    }
...
export async function assignRollNumbers(schoolId, actor, academicYearId) {
    const { roleType } = ensureActor(actor);
    if (roleType !== "ADMIN" && roleType !== "ACADEMIC_SUB_ADMIN" && roleType !== "SUPER_ADMIN") {
        throw new ApiError(403, "Forbidden");
    }
