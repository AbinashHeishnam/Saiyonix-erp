...
async function resolveStudentForCertificateRequest(
  schoolId: string,
  actor: ActorContext,
...
async function fetchStudentSnapshot(schoolId: string, studentId: string) {
  const student = await prisma.student.findFirst({
...
function buildPdfBuffer(build: (doc: PDFDoc) => void | Promise<void>) {
...
async function drawHeader(doc: PDFDoc, payload: CertificatePayload, title: string) {
...
async function uploadCertificateBuffer(buffer: Buffer, fileName: string, studentId: string) {
...
export async function requestCertificate(
  schoolId: string,
  actor: ActorContext,
  payload: CreateCertificateRequestInput
) {
  const studentId = await resolveStudentForCertificateRequest(
    schoolId,
    actor,
...
export async function listCertificateRequestsForActor(
  schoolId: string,
  actor: ActorContext,
  ...
  const mapRequests = <T extends { fileUrl?: string | null }>(items: T[]) =>
    items.map((item) => ({ ...item, fileUrl: toSecureFileUrl(item.fileUrl ?? null) }));
...
export async function listAdminCertificateRequests(schoolId: string) {
...
export async function generateTcCertificate(
  schoolId: string,
  actor: ActorContext,
  payload: AdminGenerateTcInput
) {
  const { userId } = ensureActor(actor);
