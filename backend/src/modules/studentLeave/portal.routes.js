...
const router = Router();
async function resolveLeaveAttachmentOwner(req) {
    const user = req.user;
    ...
    const studentId = typeof req.body?.studentId === "string"
        ? req.body?.studentId
        : "";
    if (!studentId) {
...
router.post("/apply", authMiddleware, allowRoles("STUDENT", "PARENT"), requirePermission("studentLeave:create"), ...uploadSingle({
    module: "leave-attachment",
    fieldName: "attachment",
    resolveUser: resolveLeaveAttachmentOwner,
}), validate(applyStudentLeaveSchema), apply);
router.get("/my", authMiddleware, allowRoles("STUDENT", "PARENT"), requirePermission("studentLeave:read"), validate({ query: listStudentLeaveQuerySchema }), myLeaves);
