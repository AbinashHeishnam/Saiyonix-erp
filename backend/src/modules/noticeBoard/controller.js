...
import { createNotice as createNoticeService, deleteNotice as deleteNoticeService, getNoticeById as getNoticeByIdService, getNoticeForActor as getNoticeForActorService, listNotices as listNoticesService, listNoticesForActor as listNoticesForActorService, updateNotice as updateNoticeService, } from "@/modules/noticeBoard/service";
...
export async function list(req, res, next) {
    try {
        ...
        const noticeType = typeof req.query.noticeType === "string" ? req.query.noticeType : undefined;
        const active = parseBoolean(req.query.active);
        const classId = typeof req.query.classId === "string" ? req.query.classId : undefined;
        const sectionId = typeof req.query.sectionId === "string" ? req.query.sectionId : undefined;
        const roleType = parseRole(req.query.roleType);
        const { items, total } = await listNoticesService(schoolId, { noticeType, active, classId, sectionId, roleType }, pagination);
        return success(res, items, "Notices fetched successfully", 200, buildPaginationMetaWithSync(total, pagination));
    }
    catch (error) {
        return next(error);
    }
...
export async function listMe(req, res, next) {
    try {
        ...
        const { items, total } = await listNoticesForActorService(schoolId, { userId, roleType }, pagination, { active });
        return success(res, items, "Notices fetched successfully", 200, buildPaginationMetaWithSync(total, pagination));
    }
    catch (error) {
        return next(error);
...
export async function getMeById(req, res, next) {
    try {
        const schoolId = getSchoolId(req);
        const id = parseId(req.params.id);
        const userId = req.user?.sub;
