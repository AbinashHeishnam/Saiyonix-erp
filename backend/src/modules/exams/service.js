...
let supportsTimetablePublishedAt = null;
...
async function resolveStudentForActor(schoolId, actor, studentIdFromPayload) {
...
async function ensureExamExists(schoolId, id) {
...
async function getExamDetailsOrThrow(schoolId, examId) {
    const exam = await prisma.exam.findFirst({
...
export async function createExam(schoolId, payload, actor) {
...
export async function addExamSubject(schoolId, examId, payload, actor) {
    const { roleType } = ensureActor(actor);
    if (!isAdminRole(roleType)) {
        throw new ApiError(403, "Forbidden");
    }
...
export async function addExamTimetable(schoolId, examId, payload, actor) {
    const { roleType } = ensureActor(actor);
    if (!isAdminRole(roleType)) {
        throw new ApiError(403, "Forbidden");
    }
    ...
    return prisma.$transaction(async (tx) => {
        ...
        const subjectClassMap = new Map(examSubjects.map((item) => [item.id, item.classSubject.classId]));
        const subjectIdMap = new Map(examSubjects.map((item) => [item.id, item.classSubjectId]));
        ...
        for (const slot of payloadSlots) {
            for (const existing of existingSlots) {
                if (existing.examSubject.classSubject.classId !== slot.classId)
                    continue;
                const existingDayKey = existing.examDate.toISOString().split("T")[0];
                if (existingDayKey !== slot.dayKey)
                    continue;
        ...
        try {
            ...
            for (const chunk of chunks) {
                const createdChunk = await Promise.all(chunk.map((item) => tx.examTimetable.create({
                    data: {
                        examSubjectId: item.examSubjectId,
                        examDate: item.examDate,
...
export async function getExamById(schoolId, examId, actor) {
    const { roleType } = ensureActor(actor);
...
export async function listExams(schoolId, filters, actor, pagination) {
    const { roleType } = ensureActor(actor);
    let where = { schoolId };
    ...
    const [items, total] = await prisma
        ...
        logger.warn(`[exams] listExams fallback: ${error instanceof Error ? error.message : "unknown error"}`);
...
export async function publishExam(schoolId, examId, actor) {
    const { roleType } = ensureActor(actor);
    if (!isAdminRole(roleType)) {
        throw new ApiError(403, "Forbidden");
    }
...
export async function lockExam(schoolId, examId, actor) {
    const { roleType } = ensureActor(actor);
    if (!isAdminRole(roleType)) {
        throw new ApiError(403, "Forbidden");
    }
...
export async function unlockExam(schoolId, examId, actor) {
    const { roleType } = ensureActor(actor);
    if (!isAdminRole(roleType)) {
        throw new ApiError(403, "Forbidden");
    }
...
export async function lockExamMarks(schoolId, examId, actor) {
...
export async function unlockExamMarks(schoolId, examId, actor) {
    const { roleType, userId } = ensureActor(actor);
    if (!isAdminRole(roleType)) {
        throw new ApiError(403, "Forbidden");
    }
...
export async function publishExamTimetable(schoolId, examId, actor) {
    const { roleType } = ensureActor(actor);
    if (!isAdminRole(roleType)) {
        throw new ApiError(403, "Forbidden");
    }
...
export async function registerForExam(schoolId, examId, actor, studentIdFromPayload) {
...
export async function listExamRegistrations(schoolId, actor, studentIdFromPayload) {
    const studentId = await resolveStudentForActor(schoolId, actor, studentIdFromPayload);
...
export async function listExamRegistrationsAdmin(schoolId, actor, examId) {
    const { roleType } = ensureActor(actor);
