...
const DEFAULT_PARENT_PASSWORD = process.env.DEFAULT_PARENT_PASSWORD ?? crypto.randomBytes(18).toString("hex");
const HEADER_KEYS = [
    "full_name",
    "registration_number",
    "admission_number",
...
function detectCsvDuplicates(parsedRows, errors) {
    ...
    for (const [, rows] of registrationMap.entries()) {
        if (rows.length > 1) {
            rows.forEach((row) => addError(errors, row.rowNumber, "Duplicate registration_number in file", row.data));
        }
    }
    for (const [, rows] of admissionMap.entries()) {
        if (rows.length > 1) {
            rows.forEach((row) => addError(errors, row.rowNumber, "Duplicate admission_number in file", row.data));
        }
    }
}
...
function buildFailedCsv(errors) {
    const headers = [
        "full_name",
        "registration_number",
        "admission_number",
...
function normalizeRow(row, errors) {
    const raw = row.data;
...
async function importRow(schoolId, roleId, passwordHash, row, academicYearId) {
    try {
...
export function buildStudentImportTemplate() {
    const header = "full_name,registration_number,admission_number,date_of_birth,gender,blood_group,parent_mobile,parent_name,class_name,section_name";
    const example = "Aru Singh,REG-001,ADM-001,2012-04-15,FEMALE,O+,9876543210,Sunita Singh,Class 5,A";
    return `${header}\n${example}`;
}
export async function previewStudentsFromCsv(schoolId, buffer, academicYearId) {
...
export async function importStudentsFromCsv(schoolId, buffer, academicYearId) {
    const rawRows = parseCsv(buffer.toString("utf8"));
    const parsedRows = buildRows(rawRows);
    const errors = [];
    const resolvedAcademicYearId = await resolveAcademicYearId(schoolId, academicYearId);
    ...
    for (const row of normalized) {
        ...
        const result = await importRow(schoolId, roleId, passwordHash, row, resolvedAcademicYearId);
        if (result.created) {
            successCount += 1;
            continue;
        }
