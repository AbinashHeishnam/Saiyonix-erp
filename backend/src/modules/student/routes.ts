...
import {
  ...
  importStudents,
  previewStudents,
} from "@/modules/studentBulkImport/controller";
import {
  createStudentSchema,
...
studentRouter.post(
  ...
  validate({ body: rollAssignSchema }),
  assignRollNumbersController
);
studentRouter.get(
  "/me",
...
studentRouter.get(
  "/:id/history",
  authMiddleware,
  allowRoles("ADMIN", "ACADEMIC_SUB_ADMIN", "SUPER_ADMIN", "TEACHER", "PARENT", "STUDENT"),
  validate({ params: studentIdParamSchema }),
  history
);
