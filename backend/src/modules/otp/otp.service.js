...
async function ensureOtpRoleLink(user, roleType, db) {
...
function buildOtpRoleCandidates(primaryRoleType) {
    const candidates = [];
    if (primaryRoleType === "STUDENT")
        candidates.push("STUDENT");
    if (primaryRoleType === "PARENT")
        candidates.push("PARENT");
    if (!candidates.includes("STUDENT"))
        candidates.push("STUDENT");
    if (!candidates.includes("PARENT"))
        candidates.push("PARENT");
    return candidates;
}
async function ensureStudentParentMatch(user, studentNumber, db) {
...
async function resolveOtpLoginContext(user, db) {
    ...
    for (const candidate of candidates) {
        try {
            ...
            else if (error.message !== "Student account not linked" &&
...
async function getOtpLoginContextOrThrow(mobile, studentNumber, db) {
...
export async function sendOtp(mobile, studentNumber, channel) {
    logger.info(`[AUTH] OTP_SEND attempt mobile=${mobile}`);
    const configuredMode = resolveOtpDeliveryMode();
    if (channel) {
        logger.info(`[OTP] requested_delivery=${channel} configured_delivery=${configuredMode} mobile=${mobile}`);
    }
...
export async function resendOtp(mobile, studentNumber, channel) {
...
export async function verifyOtp(mobile, studentNumber, otp) {
