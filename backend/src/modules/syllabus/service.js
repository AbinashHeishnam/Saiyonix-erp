...
async function ensureSyllabusExists(schoolId, id) {
...
export async function createSyllabus(schoolId, payload, actor) {
...
export async function addTopic(schoolId, syllabusId, payload, actor) {
    const { roleType } = ensureActor(actor);
...
export async function updateTopic(schoolId, topicId, payload, actor) {
    const { roleType } = ensureActor(actor);
    if (roleType !== "TEACHER" && !isAdminRole(roleType)) {
        throw new ApiError(403, "Forbidden");
    }
...
export async function deleteTopic(schoolId, topicId, actor) {
    const { roleType } = ensureActor(actor);
    if (roleType !== "TEACHER" && !isAdminRole(roleType)) {
        throw new ApiError(403, "Forbidden");
    }
...
export async function listSyllabus(schoolId, filters, actor) {
    const { roleType } = ensureActor(actor);
...
export async function completeTopic(schoolId, topicId, actor) {
...
export async function getSyllabusProgress(schoolId, syllabusId, actor) {
    const { roleType } = ensureActor(actor);
...
export async function publishSyllabus(schoolId, syllabusId, actor) {
    const { roleType } = ensureActor(actor);
    if (roleType !== "TEACHER" && !isAdminRole(roleType)) {
        throw new ApiError(403, "Forbidden");
    }
