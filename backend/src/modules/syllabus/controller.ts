...
import {
  ...
  completeTopic as completeTopicService,
  createSyllabus as createSyllabusService,
  deleteTopic as deleteTopicService,
  getSyllabusProgress as getSyllabusProgressService,
  listSyllabus as listSyllabusService,
  publishSyllabus as publishSyllabusService,
  updateTopic as updateTopicService,
} from "@/modules/syllabus/service";
import {
...
function parseTopicId(id: unknown) {
  if (typeof id !== "string") {
    throw new ApiError(400, "Invalid id");
  }
...
export async function addTopic(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    const schoolId = getSchoolId(req);
    const actor = getActor(req);
