...
import { getUnreadCount as getUnreadCountService, listNotifications as listNotificationsService, markAllRead as markAllReadService, markRead as markReadService, registerPushToken as registerPushTokenService, removePushToken as removePushTokenService, sendNotification as sendNotificationService, } from "@/modules/notification/service";
...
export async function list(req, res, next) {
    try {
        ...
        const { items, total } = await listNotificationsService(schoolId, userId, pagination);
        return success(res, items, "Notifications fetched successfully", 200, buildPaginationMetaWithSync(total, pagination));
    }
    catch (error) {
        return next(error);
    }
...
export async function send(req, res, next) {
    try {
        ...
        const idempotencyKey = typeof req.headers["idempotency-key"] === "string"
            ...
            : undefined;
        logger.info(`[trace] /notifications/send context schoolId=${schoolId} idempotencyKey=${idempotencyKey ?? "n/a"} actor=${req.user?.sub ?? req.user?.id ?? "n/a"}`);
        const data = await sendNotificationService(schoolId, req.body, req.user?.sub, idempotencyKey);
...
export async function getFirebaseWebConfig(req, res, next) {
    try {
        ...
        const expectedVapidKey = "BDQHSl8MFwDCIXC08rP5N7FRBfDYhQWtmbjBV9mSYl5p_aCM8ZMQRG3kxlxzoO7lM5bwAvD0Jb3Ggstf4v8-g10";
        ...
        return success(res, { firebaseConfig: config, vapidKey }, "Firebase web config fetched");
    }
    catch (error) {
        return next(error);
    }
...
export async function registerFcm(req, res, next) {
    try {
        ...
        if (process.env.NODE_ENV !== "production") {
            logger.info(`[FCM] register start user=${userId} school=${schoolId} token=${maskToken(token)} ua=${deviceInfo.userAgent ?? "n/a"}`);
        }
        const data = await registerPushTokenService({
            schoolId,
            userId,
