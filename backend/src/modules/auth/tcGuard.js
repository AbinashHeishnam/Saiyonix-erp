...
export function getDaysPassedSince(date) {
    return (Date.now() - date.getTime()) / MS_PER_DAY;
}
export async function getLatestApprovedTcDate(studentId) {
...
export async function getStudentTcRestriction(studentId) {
...
export async function isStudentWithinTcGrace(studentId) {
    const status = await getStudentTcRestriction(studentId);
    return status.isRestricted;
}
export async function isStudentTcLoginBlocked(studentId) {
    const status = await getStudentTcRestriction(studentId);
    return status.isExpired;
}
export async function isParentTcLoginBlocked(parentId) {
