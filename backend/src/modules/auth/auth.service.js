...
async function ensureRoleLink(user, db = prisma) {
...
async function getTeacherForSetup(userId, db = prisma) {
...
async function getAdminForSetup(userId, db = prisma) {
    const user = await db.user.findUnique({
        where: { id: userId },
        include: { role: true },
    });
...
function parseTeacherIdentifier(identifier) {
    ...
    return { channel: "phone", value: digits };
}
async function getTeacherByIdentifier(identifier, db = prisma) {
    const parsed = parseTeacherIdentifier(identifier);
    const where = parsed.channel === "email"
...
async function deliverEmailOtpForTeacher(email, otp, purpose) {
    await sendOtpEmail({
        to: email,
        otp,
        purpose,
...
async function deliverAdminSetupEmailOtp(email, otp) {
    await sendOtpEmail({
        to: email,
        otp,
        purpose: "ADMIN_SETUP",
...
async function ensureMobileAvailable(userId, mobile, db = prisma) {
...
REGISTER USER
*/
export async function registerUser({ email, password, roleType }) {
...
export async function loginUser({ email, password }) {
    ...
    */
    if (user.lockUntil && user.lockUntil > new Date()) {
        ...
        throw new ApiError(423, "Account temporarily locked due to multiple failed login attempts");
    }
    ...
    if (!isPasswordValid) {
        ...
        */
        if (attempts >= 5) {
            ...
            throw new ApiError(423, "Account locked for 10 minutes due to multiple failed login attempts");
        }
        await prisma.user.update({
            where: { id: user.id },
            data: {
...
export async function sendTeacherSetupOtp(userId, mobile) {
    ...
    const { otp, boundMobile } = await prisma.$transaction(async (tx) => {
        ...
        await ensureMobileAvailable(user.id, mobile, tx);
...
export async function verifyTeacherSetupOtp(userId, mobile, otp) {
...
export async function verifyAdminFirstLoginOtp(params) {
    ...
    await prisma.$transaction(async (tx) => {
        ...
        if (!isOtpValid) {
            ...
            throw new ApiError(400, shouldBlock ? "Too many attempts, request a new OTP" : "Invalid OTP");
        }
        await tx.emailOtpLog.update({
            where: { id: otpRecord.id },
            data: {
...
async function requestTeacherOtp(identifier, purpose, options) {
    const parsed = parseTeacherIdentifier(identifier);
    logger.info(`[AUTH] TEACHER_OTP_SEND attempt ${parsed.channel}=${parsed.value}`);
    const result = await prisma.$transaction(async (tx) => {
        const found = await getTeacherByIdentifier(identifier, tx);
        if (!found) {
...
async function verifyTeacherOtp(identifier, otp, purpose) {
    const parsed = parseTeacherIdentifier(identifier);
...
async function completeTeacherPassword(resetToken, expectedPurpose) {
    const jwtSecret = process.env.JWT_SECRET;
    if (!jwtSecret) {
        throw new ApiError(500, "JWT_SECRET is not configured");
    }
...
export async function requestTeacherActivationOtp(identifier) {
...
export async function verifyTeacherActivationOtp(identifier, otp) {
    return verifyTeacherOtp(identifier, otp, TEACHER_ACTIVATION_PURPOSE);
}
export async function completeTeacherActivation(resetToken, newPassword) {
    const { user, decoded } = await completeTeacherPassword(resetToken, TEACHER_ACTIVATION_PURPOSE);
    if (user.passwordHash) {
        const isSamePassword = await comparePassword(newPassword, user.passwordHash);
        if (isSamePassword) {
            throw new ApiError(400, "New password cannot be the same as current password");
...
export async function requestTeacherForgotPasswordOtp(identifier) {
    return requestTeacherOtp(identifier, TEACHER_RESET_PURPOSE, { strictLookup: true });
}
export async function verifyTeacherForgotPasswordOtp(identifier, otp) {
    return verifyTeacherOtp(identifier, otp, TEACHER_RESET_PURPOSE);
}
export async function completeTeacherForgotPassword(resetToken, newPassword) {
...
export async function sendPasswordResetOtpForMobile(mobile) {
...
export async function verifyPasswordResetOtpForMobile(mobile, otp) {
    const normalizedMobile = mobile.trim();
