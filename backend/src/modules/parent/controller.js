...
import { getParentProfileById, getParentProfileByUserId, listParents, updateStudentPhotoByParentUserId, updateParentProfileById, updateParentProfileByUserId, } from "@/modules/parent/service";
...
export async function adminGetParentProfile(req, res, next) {
    try {
        const schoolId = getSchoolId(req);
...
export async function adminUpdateParentProfile(req, res, next) {
    try {
        const schoolId = getSchoolId(req);
        const parentIdParam = req.params.id;
        const parentId = Array.isArray(parentIdParam) ? parentIdParam[0] : parentIdParam;
...
export async function adminListParents(req, res, next) {
    try {
        const schoolId = getSchoolId(req);
        const pagination = parsePagination(req.query);
        const { items, total } = await listParents(schoolId, pagination);
        return success(res, items, "Parents fetched successfully", 200, buildPaginationMeta(total, pagination));
    }
    catch (error) {
        return next(error);
    }
}
export async function getParentChildIdCard(req, res, next) {
    try {
        const schoolId = getSchoolId(req);
        const userId = req.user?.sub;
        if (!userId) {
...
export async function uploadParentStudentPhoto(req, res, next) {
    try {
        const schoolId = getSchoolId(req);
        const roleType = req.user?.roleType;
        if (roleType !== "PARENT") {
        ...
        const data = await updateStudentPhotoByParentUserId(schoolId, userId, studentId, uploadedFile.fileUrl);
