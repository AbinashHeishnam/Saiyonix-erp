...
const EMAIL_OTP_RESEND_COOLDOWN_MS = env.EMAIL_OTP_RESEND_COOLDOWN_SECONDS * 1000;
...
async function ensureStaffRoleLink(user, db) {
...
async function getEmailOtpUserOrThrow(email, db) {
...
async function deliverEmailOtp(email, otp, purpose) {
    await sendOtpEmail({
        to: email,
        otp,
        purpose,
...
async function upsertEmailOtp(db, params) {
...
export async function verifyEmailOtp(email, otp) {
    ...
    const { user } = await prisma.$transaction(async (tx) => {
        ...
        await ensureStaffRoleLink(user, tx);
