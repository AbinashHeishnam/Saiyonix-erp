...
async function ensureCircularExists(schoolId, id) {
...
export async function createCircular(schoolId, payload, createdById) {
    try {
...
export async function listCirculars(schoolId, filters, pagination) {
    const now = new Date();
    const targetFilters = [];
    if (filters.classId) {
        targetFilters.push({
...
export async function getCircularById(schoolId, id) {
    const circular = await prisma.circular.findFirst({
        where: { id, schoolId },
    });
    if (!circular) {
...
export async function updateCircular(schoolId, id, payload, updatedById) {
...
export async function deleteCircular(schoolId, id, deletedById) {
