...
async function getSenderNameForRoom(schoolId, senderId, senderRole, sectionId) {
...
async function resolveClassSubjectId(schoolId, classId, subjectId) {
...
async function listClassroomAnnouncements(schoolId, classId, sectionId) {
...
export async function getTeacherClassroom(schoolId, userId, teacherId) {
...
async function resolveStudentEnrollment(schoolId, studentId) {
    const academicYearId = await getActiveAcademicYearId(schoolId);
    if (!academicYearId) {
        throw new ApiError(400, "Active academic year not found");
    }
...
async function getStudentIdForParent(schoolId, userId, studentId) {
    const parent = await prisma.parent.findFirst({
...
async function upsertChatRoom(classId, sectionId, subjectId) {
...
async function ensureChatRoomAccess(schoolId, actor, room) {
...
export async function canJoinChatRoomSafe(schoolId, actor, room) {
    try {
...
export async function getStudentClassroom(schoolId, userId, roleType, studentId) {
    ...
    const submittedBySubject = new Map();
    assignmentList.forEach((assignment) => {
        totalBySubject.set(assignment.classSubjectId, (totalBySubject.get(assignment.classSubjectId) ?? 0) + 1);
        if (submittedSet.has(assignment.id)) {
            submittedBySubject.set(assignment.classSubjectId, (submittedBySubject.get(assignment.classSubjectId) ?? 0) + 1);
        }
    });
    const assignmentTeacherBySubject = new Map();
    ...
    return classSubjects.map((subject) => {
        ...
        const primaryTeacher = assignmentTeacherBySubject.get(subject.id) ??
            teachers[0] ??
            null;
        return {
            ...
            pendingAssignments: (totalBySubject.get(subject.id) ?? 0) -
                (submittedBySubject.get(subject.id) ?? 0),
        };
    });
}
async function ensureTeacherSectionAccess(schoolId, teacherId, sectionId) {
...
export async function getSectionClassroom(schoolId, userId, roleType, sectionId) {
    if (roleType !== "TEACHER") {
        throw new ApiError(403, "Forbidden");
    }
    const teacher = await prisma.teacher.findFirst({
    ...
    const uniqueTeachers = new Map(teachers.map((item) => [item.teacher.id, {
            id: item.teacher.id,
    ...
    return {
        ...
        parents: parentLinks.reduce((acc, link) => {
            if (!link.parent)
                return acc;
...
export async function getSubjectClassroom(schoolId, userId, roleType, classSubjectId, studentId, pagination) {
    let sectionId = null;
    let resolvedStudentId = null;
    ...
    const timetableTeachers = teacherLinks.length === 0
    ...
    const submissions = resolvedStudentId && assignments.length
        ? await prisma.assignmentSubmission.findMany({
            where: {
    ...
    const section = sectionId
        ...
        : null;
    const room = sectionId && subject.subjectId
    ...
    return {
        ...
        announcements: await listClassroomAnnouncements(schoolId, subject.classId, sectionId),
    };
}
export async function createAssignmentInClassroom(schoolId, actor, payload) {
    const classSubjectId = await resolveClassSubjectId(schoolId, payload.classId, payload.subjectId);
    const assignment = await createAssignment(schoolId, {
        classSubjectId,
    ...
    if (payload.fileUrl) {
        ...
        await addAssignmentAttachment(schoolId, assignment.id, {
            fileName,
...
export async function createNoteInClassroom(schoolId, actor, payload) {
    const classSubjectId = await resolveClassSubjectId(schoolId, payload.classId, payload.subjectId);
    const note = await createNote(schoolId, {
        classSubjectId,
        sectionId: payload.sectionId ?? null,
        title: payload.title,
        description: payload.description ?? null,
...
export async function createAnnouncementInClassroom(schoolId, actor, payload) {
    if (!actor.userId || actor.roleType !== "TEACHER") {
        throw new ApiError(403, "Forbidden");
    }
    const teacher = await prisma.teacher.findFirst({
...
export async function submitAssignmentInClassroom(schoolId, actor, payload) {
    const submission = await submitAssignment(schoolId, payload.assignmentId, { submissionUrl: payload.submissionUrl }, actor, payload.studentId);
...
export async function getChatRoomMessages(schoolId, actor, roomId, options) {
    ...
    const teacherIds = new Set(orderedMessages
        .filter((msg) => msg.senderRole === "TEACHER")
        .map((msg) => msg.senderId));
    const studentIds = new Set(orderedMessages
        .filter((msg) => msg.senderRole === "STUDENT")
        .map((msg) => msg.senderId));
    const parentIds = new Set(orderedMessages
    ...
    const parentLinks = parentIds.size
        ? await prisma.parentStudentLink.findMany({
            where: {
        ...
        : [];
    const parentStudentMap = new Map(parentLinks.map((link) => [link.parentId, link.student.fullName]));
    ...
    const nextCursor = enriched.length && enriched.length >= limit
        ? enriched[0].createdAt?.toISOString?.() ?? null
        : null;
    return { room, messages: enriched, nextCursor };
}
export async function sendChatRoomMessage(schoolId, actor, roomId, payload) {
    if (!actor.userId || !actor.roleType) {
        throw new ApiError(401, "Unauthorized");
    }
    const room = await prisma.chatRoom.findFirst({
    ...
    const senderName = await getSenderNameForRoom(schoolId, actor.userId, actor.roleType, room.sectionId);
    let replyToName = null;
    if (saved.replyTo) {
        replyToName = await getSenderNameForRoom(schoolId, saved.replyTo.senderId, saved.replyTo.senderRole, room.sectionId);
    }
    return {
        ...saved,
...
export async function markChatMessageSeen(schoolId, actor, messageId) {
    if (!actor.userId || !actor.roleType) {
        throw new ApiError(401, "Unauthorized");
    }
...
export async function pinChatRoomMessage(schoolId, actor, messageId, pin) {
    if (!actor.userId || !actor.roleType) {
        throw new ApiError(401, "Unauthorized");
    }
    if (actor.roleType !== "TEACHER") {
