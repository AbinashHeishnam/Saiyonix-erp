...
import { teacherBulkImportRowSchema, } from "@/modules/teacherBulkImport/validation";
const MAX_BULK_IMPORT_ROWS = 500;
const DEFAULT_TEACHER_PASSWORD = process.env.DEFAULT_TEACHER_PASSWORD ?? crypto.randomBytes(18).toString("hex");
const HEADER_KEYS = [
    "firstName",
    "lastName",
    "email",
...
function addError(errors, rowNumber, message, data) {
...
function mapValidationMessage(messages) {
...
function validateRow(row, errors) {
    ...
    if (!result.success) {
        const message = mapValidationMessage(result.error.issues.map((issue) => issue.message));
        addError(errors, row.rowNumber, message, row.data);
        return null;
    }
    return result.data;
}
function generateEmployeeId(rowNumber) {
...
async function ensureUniqueUser(tx, params) {
...
function detectCsvDuplicates(parsedRows, errors) {
    ...
    for (const [, rows] of emailMap.entries()) {
        if (rows.length > 1) {
            rows.forEach((row) => addError(errors, row.rowNumber, "Duplicate in file", row.data));
        }
    }
...
export function buildTeacherImportTemplate() {
    const header = "firstName,lastName,email,phone,gender,qualification,experienceYears,address";
    const example = "John,Doe,john@example.com,9876543210,Male,B.Ed,5,Imphal East";
    return `${header}\n${example}`;
}
export async function previewTeachersFromCsv(schoolId, buffer) {
    ...
    const validRows = parsedRows.length - new Set(invalidRows.map((row) => row.row)).size;
    return {
...
export async function importTeachersFromCsv(schoolId, buffer) {
    const rawRows = parseCsv(buffer.toString("utf8"));
    const parsedRows = buildRows(rawRows);
