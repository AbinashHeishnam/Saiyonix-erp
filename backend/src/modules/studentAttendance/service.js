...
async function resolveTeacherId(client, schoolId, params) {
...
async function ensureTeacherIsClassTeacher(client, sectionId, teacherId, attendanceDate, timeZone) {
    const section = await client.section.findFirst({
...
export async function markStudentAttendance(schoolId, payload, actor) {
    try {
        ...
        return await prisma.$transaction(async (tx) => {
            ...
            if (actor.roleType === "TEACHER") {
                await ensureTeacherIsClassTeacher(db, payload.sectionId, teacherId, attendanceDate, timeZone);
            }
...
export async function listStudentAttendance(schoolId, filters, pagination) {
    const where = {
...
export async function getStudentAttendanceById(schoolId, id) {
...
export async function updateStudentAttendance(schoolId, id, payload, actor) {
    try {
        if (actor.roleType === "TEACHER") {
