...
export async function teacherSubstitutionsToday(req, res, next) {
    try {
...
export async function adminSubstitutions(req, res, next) {
    try {
        const schoolId = getSchoolId(req);
        ...
        const { items, total, availability, approvedLeaves } = await listSubstitutionsAdmin({
            ...
            academicYearId: typeof req.query?.academicYearId === "string" ? req.query.academicYearId : null,
            includeAvailability: req.query?.includeAvailability === "true" || req.query?.includeAvailability === "1",
            pagination,
        });
        return success(res, { items, availability, approvedLeaves }, "Substitutions fetched successfully", 200, buildPaginationMeta(total, pagination));
    }
    catch (error) {
        return next(error);
    }
