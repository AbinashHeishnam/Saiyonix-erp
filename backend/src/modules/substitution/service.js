...
export async function generateSubstitutions(client, params) {
    ...
    for (const leave of teacherLeaves) {
        ...
        for (let d = leaveFrom; d <= leaveTo; d = addDaysUTC(d, 1)) {
            ...
            const set = leaveSetByDate.get(dk) ?? new Set();
    ...
    const assignmentByDayPeriod = new Map();
    const workloadByTeacherDay = new Map();
    for (const slot of timetableAssignments) {
        if (!slot.teacherId)
            continue;
        const key = `${slot.dayOfWeek}:${slot.periodId}`;
        const set = assignmentByDayPeriod.get(key) ?? new Set();
    ...
    const existingSubBySlot = new Map();
    const unavailableMap = new Set();
    const workloadByTeacherDate = new Map();
    const teacherIdsSorted = [...teacherIds].sort((a, b) => a.localeCompare(b));
    const unavailableKey = (teacherId, dk, periodId) => `${teacherId}:${dk}:${periodId}`;
    ...
    for (const row of subjectTeachers) {
        if (!row.classSubject?.subjectId)
            continue;
        const set = subjectTeacherMap.get(row.classSubject.subjectId) ?? new Set();
    ...
    for (const date of dateRange) {
        ...
        const dk = formatLocalDateKey(date, timeZone);
        const assignedTeacherForDay = new Set();
        ...
        for (const slot of slots) {
            ...
            const pickCandidate = (subjectOnly) => {
                let bestId = null;
                ...
                for (const teacherId of teacherIdsSorted) {
                    ...
                    if (assignedTeacherForDay.has(teacherId))
                        continue;
                    if (unavailableMap.has(unavailableKey(teacherId, dk, slot.periodId)))
                        continue;
...
export async function getTeacherSubstitutionsToday(schoolId, userId) {
...
export async function listSubstitutionsAdmin(params) {
    const resolvedAcademicYearId = params.academicYearId ?? (await getActiveAcademicYearId(prisma, params.schoolId));
    const where = {
    ...
    if (params.includeAvailability && dateOnly) {
        ...
        for (const slot of slots) {
            if (!slot.teacherId)
                continue;
            const set = slotMap.get(slot.periodId) ?? new Set();
            set.add(slot.teacherId);
        ...
        for (const sub of subs) {
            if (!sub.substituteTeacherId)
                continue;
            const set = subMap.get(sub.periodId) ?? new Set();
