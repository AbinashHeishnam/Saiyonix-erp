...
function buildPaths(relativePath) {
...
export function getAdmitCardPaths(examId, studentId, salt) {
...
export function getReportCardPaths(examId, studentId) {
    const relative = path.posix.join("report-cards", examId, `${studentId}.pdf`);
    return buildPaths(relative);
}
export function getSignedUrl(publicUrl) {
