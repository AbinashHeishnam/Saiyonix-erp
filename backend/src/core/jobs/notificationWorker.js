...
function setWorkerStatus(status, error) {
...
async function handleFailure(jobId, payload, error) {
    ...
    const nextAttemptAt = nextStatus === "PENDING" ? getNextAttemptAt(attempts).toISOString() : null;
...
export async function processNextNotificationJob() {
    ...
    if (!payload) {
        await prisma.notificationJob.update({
            where: { id: job.id },
            data: { status: "FAILED", payload: job.payload },
        });
...
async function cleanupFailedJobs(retentionDays) {
