...
import { aggregateNotificationDelivery, cleanupPushTokens, deliverQueuedNotification, markNotificationDeliveryFailed, } from "@/services/notificationService";
...
export async function getJobWorker() {
    ...
    jobWorker = new Worker("saiyonix-jobs", async (job) => {
        const payload = job.data;
        console.log(`[job:start] ${job.name} ${job.id}`);
        try {
            switch (payload.type) {
                case "RESULTS_RECOMPUTE":
                    await withConsoleTime(`job:results:${payload.examId}`, async () => withTimeout(recomputeResults(payload.schoolId, payload.examId, {
                        userId: "SYSTEM",
                        roleType: "ADMIN",
                    }), 60_000, "Results recompute"));
                    break;
                case "RESULTS_PUBLISH":
                    await withConsoleTime(`job:results-publish:${payload.examId}`, async () => withTimeout(publishResults(payload.schoolId, payload.examId, payload.actor), 120_000, "Results publish"));
                    break;
                case "RANKING_RECOMPUTE":
                    await withConsoleTime(`job:ranking:${payload.examId}`, async () => withTimeout(recomputeRanking(payload.schoolId, payload.examId, {
                        userId: "SYSTEM",
                        roleType: "ADMIN",
                    }), 60_000, "Ranking recompute"));
                    break;
                case "ADMIT_CARD_GENERATE":
                    await withConsoleTime(`job:admit:${payload.examId}`, async () => withTimeout(generateAdmitCardsForExam(payload.schoolId, payload.examId), 60_000, "Admit card generation"));
                    break;
                case "ADMIT_CARD_PDF_GENERATE":
                    await withConsoleTime(`job:admit-pdf:${payload.examId}`, async () => withTimeout(bulkGeneratePDFs(payload.schoolId, payload.examId), 120_000, "Admit card PDF generation"));
                    break;
                case "REPORT_CARD_PDF_GENERATE":
                    await withConsoleTime(`job:report-card-pdf:${payload.examId}:${payload.studentId}`, async () => withTimeout(generateReportCardPdf(payload.schoolId, payload.examId, payload.studentId), 120_000, "Report card PDF generation"));
                    break;
                case "ASSIGNMENT_REMINDER":
                    await withConsoleTime(`job:assignment-reminder:${payload.schoolId}`, async () => withTimeout(checkAndSendAssignmentReminders(payload.schoolId), 30_000, "Assignment reminders"));
                    break;
                ...
                case "PUSH_TOKEN_CLEANUP":
                    await withConsoleTime(`job:push-token-cleanup:${payload.schoolId}`, async () => withTimeout(cleanupPushTokens({ schoolId: payload.schoolId }), 30_000, "Push token cleanup"));
                    break;
                default:
                    break;
            }
    ...
    jobWorker.on("failed", (job, err) => {
        ...
        console.error(`[job:fail] ${job?.name} ${job?.id} attempts=${job?.attemptsMade} durationMs=${durationMs ?? "n/a"}`, err);
    });
    ...
    jobWorker.on("completed", (job) => {
        const durationMs = job?.processedOn && job?.finishedOn ? job.finishedOn - job.processedOn : undefined;
        console.log(`[job:duration] ${job?.name} ${job?.id} durationMs=${durationMs ?? "n/a"}`);
    });
    setInterval(async () => {
        try {
            ...
            console.log(`[queue:stats] waiting=${counts.waiting} active=${counts.active} delayed=${counts.delayed} failed=${counts.failed}`);
        }
        catch (error) {
...
export async function getNotificationWorker() {
    ...
    notificationWorker = new Worker("notification", async (job) => {
        try {
            ...
            try {
                logger.info(`[trace] 🔥 WORKER RECEIVED JOB queue=notification name=${job.name} id=${job.id} data=${JSON.stringify(job.data ?? null)}`);
            }
            catch {
                logger.info(`[trace] 🔥 WORKER RECEIVED JOB queue=notification name=${job.name} id=${job.id} data=[unserializable]`);
            }
            if (job.name === "deliver-notification") {
                const data = job.data;
                const notificationId = typeof data?.notificationId === "string" ? data.notificationId.trim() : "";
                console.log(`[notification-worker:received] name=${job.name} id=${job.id} notificationId=${notificationId || "n/a"}`);
                if (!notificationId) {
                    throw new Error("notificationId is required for deliver-notification");
                }
                await deliverQueuedNotification({ notificationId });
                console.log(`[notification-worker:processed] name=${job.name} id=${job.id} notificationId=${notificationId}`);
                return;
            }
            if (job.name !== "notify") {
            ...
            if (typeof data.notificationId === "string" && data.notificationId.trim().length > 0) {
                console.log(`[notification-worker:received] name=${job.name} id=${job.id} notificationId=${data.notificationId.trim()}`);
                await deliverQueuedNotification({ notificationId: data.notificationId });
                console.log(`[notification-worker:processed] name=${job.name} id=${job.id} notificationId=${data.notificationId.trim()}`);
                return;
            }
            const users = data.userIds ?? (data.userId ? [data.userId] : []);
            const validUsers = (users || []).filter((id) => typeof id === "string" && id.trim().length > 0);
            ...
            console.log(`[notification-worker:received] name=${job.name} id=${job.id} users=${validUsers.length} schoolId=${schoolId}`);
            await sendPush({
                schoolId,
                userIds: validUsers,
                title,
            ...
            console.log(`[notification-worker:processed] name=${job.name} id=${job.id} users=${validUsers.length} schoolId=${schoolId}`);
        }
        catch (err) {
    ...
    notificationWorker.on("failed", async (job, err) => {
        try {
            ...
            if (data?.notificationId && isFinalFailure) {
                try {
                    await notificationDlqQueue?.add("FAILED_NOTIFICATION", {
                ...
                await markNotificationDeliveryFailed(data.notificationId, err instanceof Error ? err.message : String(err));
            }
        }
        catch (error) {
