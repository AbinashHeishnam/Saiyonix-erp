...
const pool = new Pool({
    ...
    connectionTimeoutMillis: Number.isFinite(poolTimeoutSeconds) && poolTimeoutSeconds > 0
...
function isUnboundedFindMany(args) {
    if (!args)
        return false;
...
export function enforceQueryLimits(args) {
    if (!args)
        return args;
