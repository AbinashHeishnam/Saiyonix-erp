...
export function extractTokenFromRequest(req: Request) {
  ...
  const queryToken =
    req.originalUrl.includes("/files/secure") &&
    typeof (req as Request & { query?: Record<string, unknown> }).query?.token === "string"
      ? ((req as Request & { query?: Record<string, unknown> }).query?.token as string)
      : null;
...
const userCache = new Map<string, { data: { mustChangePassword: boolean; isMobileVerified: boolean; isActive: boolean } | null; time: number }>();
const AUTH_CACHE_TTL_MS = 3000;

export async function updateAuthUserCache(
  userId: string,
  data: { mustChangePassword: boolean; isMobileVerified: boolean; isActive: boolean }
) {
...
export async function authMiddleware(
  ...
  try {
    ...
    if (
      ...
      const isAllowedSetupRoute =
        req.originalUrl.startsWith("/api/v1/auth/admin-setup") ||
    ...
    if (
      ...
      if (!csrfHeader || !validateCsrfToken((req.user as { id?: string }).id ?? "", csrfHeader as string)) {
