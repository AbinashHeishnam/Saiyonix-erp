...
export function initSocket(httpServer) {
    ...
    async function ensureRoomAccess(user, roomId) {
        ...
        const allowed = await canJoinChatRoomSafe(user.schoolId, { userId: user.userId, roleType: user.roleType }, {
            roomId: room.id,
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("send_message", async (data) => {
            try {
                ...
                const saved = await sendChatRoomMessage(payload.schoolId, { userId: payload.userId, roleType: payload.roleType }, roomId, { message, fileUrl, replyToId });
        ...
        socket.on("message_seen", async (data) => {
            try {
                ...
                if (!room)
                    return;
                const seen = await markChatMessageSeen(payload.schoolId, { userId: payload.userId, roleType: payload.roleType }, messageId);
                if (seen) {
