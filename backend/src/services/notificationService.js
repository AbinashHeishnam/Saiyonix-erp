...
function buildDeliveryPayload(notification, recipient) {
    return {
...
function classifyFcmErrorCode(code) {
    const c = (code ?? "").toLowerCase();
    const invalid = c.includes("registration-token-not-registered") ||
...
async function invalidateToken(pushTokenId, reason) {
...
async function markRecipientStatus(recipientId, status) {
...
export async function registerPushToken(input) {
    ...
    const platform = isExpoToken ? "EXPO" : input.platform;
    ...
    try {
        ...
        logger.info(`[PUSH_TOKEN] stored user=${input.userId} school=${input.schoolId} platform=${stored.platform} activeExpoTokens=${activeCount} token=${token.slice(0, 8)}…`);
    }
    catch (logError) {
...
export async function queueNotificationDelivery(notificationId) {
    ...
    await queue.add("deliver-notification", { notificationId }, {
...
export async function deliverQueuedNotification(job) {
    ...
    const alreadySentExpo = new Set();
    const alreadySentFcm = new Set();
    for (const log of sentLogs) {
        if (!log.pushTokenId)
            continue;
        if (log.channel === "MOBILE_PUSH")
            alreadySentExpo.add(log.pushTokenId);
        if (log.channel === "WEB_PUSH")
            alreadySentFcm.add(log.pushTokenId);
    }
    if (!wantsPush) {
    ...
    if (expoItems.length > 0) {
        ...
        const sendExpo = async (messages) => {
            ...
            for (let index = 0; index < tickets.length; index += 1) {
                ...
                if (!ticket || ticket.status !== "ok" || !("id" in ticket) || !ticket.id)
                    continue;
                receiptIds.push(ticket.id);
                const message = messages[index];
                const token = message && typeof message.to === "string"
                    ? message.to
                    : null;
                if (token) {
                    ...
                    for (const [id, receipt] of Object.entries(receipts)) {
                        if (receipt && typeof receipt === "object" && "status" in receipt && receipt.status === "error") {
                            ...
                            console.error("❌ PUSH RECEIPT ERROR:", {
                                id,
                                message: "message" in receipt ? receipt.message : undefined,
                                details: "details" in receipt ? receipt.details : undefined,
                            });
                        }
                        else {
        ...
        for (const [project, items] of groups) {
            ...
            for (const chunk of expoClient.chunkPushNotifications(messages)) {
                ...
                try {
                    ...
                    await Promise.all(chunkItems.map(async (item) => {
                        ...
                        try {
                            await createLog({
                                ...
                                platform: "EXPO",
                                status: "FAILED",
                                errorCode: error instanceof Error && error.message === "Push timeout"
                                    ? "PUSH_TIMEOUT"
                                    : "EXPO_PUSH_FAILED",
                ...
                await Promise.all(tickets.map(async (ticket, index) => {
                    ...
                    const errorCode = details && typeof details === "object" && "error" in details
    ...
    if (fcmGroups.length > 0) {
        ...
        if (!messaging) {
            for (const group of fcmGroups) {
                ...
                await Promise.all(group.tokens.map((item) => createLog({
                    schoolId: notification.schoolId,
                    notificationId: notification.id,
            ...
            await Promise.all(fcmGroups.map((group) => limit(async () => {
                const stats = getStats(group.recipientId);
                ...
                let batch;
                try {
                    batch = await withTimeout(messaging.sendEachForMulticast({
                        tokens,
                        notification: {
                            title: notification.title,
                            body: notification.body,
                ...
                await Promise.all(batch.responses.map(async (resp, index) => {
                    ...
                    stats.failed += 1;
                    const rawCode = resp.error && typeof resp.error === "object" && "code" in resp.error
...
export async function markNotificationDeliveryFailed(notificationId, reason) {
    await prisma.notificationRecipient.updateMany({
        where: {
            notificationId,
...
export async function cleanupPushTokens(params) {
...
export async function aggregateNotificationDelivery(params) {
    ...
    for (const row of logs) {
        if (row.status === "SENT")
            out.sent = row._count.status;
        if (row.status === "FAILED")
            out.failed = row._count.status;
        if (row.status === "INVALID_TOKEN")
            out.invalid = row._count.status;
    }
