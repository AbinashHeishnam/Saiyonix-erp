...
function normalizeMobileForLog(input) {
...
export async function sendPhoneOtp(mobile, otp, requestedMode) {
    ...
    if (requestedMode && requestedMode !== configured.mode) {
        logger.warn(`[OTP] delivery_mode_mismatch requested=${requestedMode} configured=${configured.mode} mobile=${normalizedMobile}`);
    }
    logger.info(`[OTP] delivery_attempt mode=${mode} provider=${provider} mobile=${normalizedMobile}`);
    try {
        ...
        logger.info(`[OTP] delivery_success mode=${mode} provider=${provider} mobile=${normalizedMobile}`);
    }
    catch (error) {
        const message = error instanceof Error ? error.message : "Unknown error";
        logger.error(`[OTP] delivery_failed mode=${mode} provider=${provider} mobile=${normalizedMobile} error=${message}`);
        throw error;
    }
}
