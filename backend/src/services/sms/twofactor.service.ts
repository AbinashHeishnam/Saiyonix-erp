...
export async function sendPhoneOtp(phone: string, otp: string, mode: OtpDeliveryMode) {
  ...
  try {
    ...
    const data = response.data as { Status?: string; Details?: string };
    const status = data?.Status ?? "Unknown";
    const details = data?.Details ?? "No details";

    logger.info(
      `[OTP][2FACTOR] mode=${mode} status=${status} details=${details} template=${templateName} mobile=${mobile}`
    );
