...
import { PrismaClient, UserRole, AttendanceStatus, NoticeTargetType } from "@prisma/client";
...
async function seedRolePermissions() {
  ...
  for (const roleType of Object.keys(rolePermissionMap) as UserRole[]) {
...
async function seedAdmin(schoolId: string) {
...
async function seedAcademicYear(schoolId: string) {
...
async function seedClasses(schoolId: string, academicYearId: string) {
  const classes = [];
...
async function seedSections(classes: Array<{ id: string }>) {
  const sectionNames = ["A", "B"];
  const sections: Array<{ id: string; classId: string; sectionName: string }> = [];
...
async function seedPeriods(schoolId: string) {
  const periods = [
...
async function seedTeachers(schoolId: string) {
  const role = await prisma.role.findUnique({
...
async function assignClassTeachers(
...
async function seedTimetableSlots(params: {
  sections: Array<{ id: string; classId: string }>;
  classSubjectIds: Map<string, string[]>;
  periodIds: string[];
  ...
  for (const section of params.sections) {
    const subjects = params.classSubjectIds.get(section.classId) ?? [];
    let subjectIndex = 0;
    for (const day of dayOfWeekValues) {
      for (const periodId of params.periodIds) {
...
async function seedParentsAndStudents(params: {
  schoolId: string;
  academicYearId: string;
  sections: Array<{ id: string; classId: string }>;
  studentCount: number;
  ...
  const students: Array<{ id: string; sectionId: string; classId: string }> = [];
  const parentIds: string[] = [];

  for (let i = 1; i <= Math.ceil(params.studentCount / 2); i += 1) {
  ...
  for (let i = 1; i <= params.studentCount; i += 1) {
    const section = params.sections[(i - 1) % params.sections.length];
...
async function seedAttendance(params: {
  academicYearId: string;
  students: Array<{ id: string; sectionId: string }>;
  sections: Array<{ id: string; classTeacherId: string | null }>;
  timetableSlots: Array<{ id: string; sectionId: string }>;
}) {
  const today = new Date();
  const attendanceDate = new Date(Date.UTC(today.getUTCFullYear(), today.getUTCMonth(), today.getUTCDate()));

  const teacherBySection = new Map(
    params.sections.map((section) => [section.id, section.classTeacherId])
  );
  const slotBySection = new Map<string, string>();
...
async function seedExamAndMarks(params: {
  schoolId: string;
  academicYearId: string;
  ...
  const class1 = Array.from(params.classSubjects.keys())[0];
  const subjectId = params.classSubjects.get(class1)?.[0];
  if (!subjectId) return;
  ...
  for (const student of params.students.filter((s) => s.classId === class1)) {
...
async function seedNotices(schoolId: string) {
  const now = new Date();
...
async function validateData(params: {
  ...
  const missingTeachers = params.sections.filter((section) => !section.classTeacherId);
  ...
  const marks = await prisma.mark.findMany({
    include: {
      student: {
        include: { enrollments: { where: { academicYearId: params.academicYearId } } },
      },
    },
  });
  for (const mark of marks) {
    const enrollment = mark.student.enrollments[0];
    const section = params.sections.find((item) => item.id === enrollment?.sectionId);
