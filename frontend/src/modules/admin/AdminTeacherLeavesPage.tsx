...
type TeacherLeave = {
  id: string;
  teacherId: string;
  teacher?: { fullName?: string | null; employeeId?: string | null; photoUrl?: string | null };
  fromDate: string;
  toDate: string;
  reason: string;
...
export default function AdminTeacherLeavesPage() {
  ...
  const { data, loading, error, refresh } = useAsync(async () => {
    const res = await api.get("/admin/teacher-leaves", { params: { page: 1, limit: 200 } });
  ...
  const handleUpdate = async (leaveId: string, status: "APPROVED" | "REJECTED") => {
    ...
    try {
      await safeApiCall(
        () => api.patch(`/admin/teacher-leave/${leaveId}`, { status, remarks }),
        {
    ...
    <div className="flex flex-col gap-6 animate-slide-up">
      ...
      <Card>
        <div className="flex flex-col gap-4">
          <Input
            label="Search"
            placeholder="Search by teacher, reason, or status"
            value={query}
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-semibold text-ink-900">
                        {leave.teacher?.fullName ?? "Teacher"}
                      </p>
                      <p className="text-xs text-ink-500">
                        {leave.teacher?.employeeId ?? "Employee ID"} •{" "}
