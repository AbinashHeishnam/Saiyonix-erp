...
import { getParentPromotionView } from "../../../services/api/promotion";
...
export default function ParentPromotionView() {
  const { data, loading, error } = useAsync<PromotionViewPayload | null>(async () => {
    const res = await getParentPromotionView();
  ...
  return (
    <div className="flex flex-col gap-6 animate-slide-up">
      <PageHeader title="Promotion Update" subtitle="Track your child’s promotion outcome." />

      <Card title="Promotion Status" subtitle="Latest promotion summary for your child.">
        {loading ? (
          <Loader label="Fetching promotion details..." />
        ) : error ? (
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className={`rounded-2xl border p-4 text-sm ${effectiveStatus === "PROMOTED" || effectiveStatus === "COMPLETED" ? "border-jade-200 bg-jade-50/60 text-jade-700" : effectiveStatus === "NOT_PROMOTED" || effectiveStatus === "FAILED" ? "border-rose-200 bg-rose-50/60 text-rose-700" : "border-amber-200 bg-amber-50/60 text-amber-700"}`}>
