...
export default function ParentLeavePage() {
  ...
  const items = useMemo(() => {
    ...
    return list.filter((leave) =>
      [leave.student?.fullName, leave.reason, leave.leaveType, leave.status]
        .filter(Boolean)
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (!form.studentId || !form.fromDate || !form.toDate || !form.reason.trim()) {
        setError("Student, From Date, To Date, and Reason are required.");
        return;
      }
      ...
      await safeApiCall(
        () => api.post("/student/leave/apply", formData),
  ...
  return (
    <div className="flex flex-col gap-6 animate-slide-up">
      <PageHeader title="Student Leaves" subtitle="Apply leave for your children." />
      ...
      <Card title="Apply Leave">
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          >
            ...
            {(profile?.students ?? []).map((student: ParentProfileStudent) => (
        ...
        <div className="mt-4 flex items-center gap-3">
          <Button onClick={handleSubmit} loading={saving} disabled={!form.studentId}>
            Submit Leave
          </Button>
